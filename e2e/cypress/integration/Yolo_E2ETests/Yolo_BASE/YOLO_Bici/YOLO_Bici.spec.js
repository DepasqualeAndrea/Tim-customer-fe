...
describe('T-21 Reachability form bici with login', () => {
    ...
    it('T-21 Reaches Yolo home page', () => {
        cy.visit(env.url)
        cy.wait(5000)
    ...
    it('T-21 Clicks ok on cookie banner', () => {
        cy.get('.cc-btn').click()
    })


    it('T-21 Reaches login page', () => {
        cy.get('.user-btn > .nav-link').click()
        cy.url().should('equal', env.url + '/login')
    })


    it('T-21 Compiles login form', () => {
    ...
    it('T-21 Reaches form bici page', () => {
        cy.get('.nav > :nth-child(3) > .nav-link').click()
        cy.get(':nth-child(6) > .flex-column > .go-to-arrow').click()
        cy.url().should('equal', env.url + '/product-landing;code=ea-bike-easy,ea-bike-top')
    })
