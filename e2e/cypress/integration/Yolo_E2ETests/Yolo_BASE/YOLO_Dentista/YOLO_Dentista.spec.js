...
describe('T-1 Reachability form dentista with login', () => {
  ...
  it('T-1 Clicks ok on cookie banner', () => {
    cy.get('.cc-btn').click()
  })


  it('T-1 Reaches login page', () => {
    cy.get('.user-btn > .nav-link').click()
    cy.url().should('equal', env.url + '/login')
  })


  it('T-1 Compiles login form', () => {
  ...
  it('T-13 Reaches dentista product',()=>{
    cy.visit(env.url + '/preventivatore;code=ba-dentist')
    cy.wait(5000)
})


  it('T-1 Reaches form dentista', () => {
   // cy.get('.nav > :nth-child(2) > .nav-link').click()
    //cy.get(':nth-child(1) > .flex-column > .go-to-arrow').click()
    cy.url().should('equal', env.url + '/preventivatore;code=ba-dentist')
  })
