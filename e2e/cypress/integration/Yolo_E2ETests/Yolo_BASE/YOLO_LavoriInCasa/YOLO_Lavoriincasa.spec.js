...
describe('T-18 Reachability lavori in casa page with login', () => {
  ...
  it('T-18 Reaches Yolo home page', () => {
    cy.visit(env.url)
    cy.wait(5000)
  ...
  it('T-18 Clicks ok on cookie banner', () => {
    cy.get('.cc-btn').click()
  })


  it('T-18 Reaches login page', () => {
    cy.get('.user-btn > .nav-link').click()
    cy.url().should('equal', env.url + '/login')
  })


  it('T-18 Compiles login form', () => {
  ...
  it('T-18 Reaches lavori in casa page', () => {
    cy.get('.nav > :nth-child(3) > .nav-link').click()
    cy.get(':nth-child(3) > .flex-column > .go-to-arrow').click()
    cy.url().should('equal', env.url + '/preventivatore;code=chubb-work')
  })
