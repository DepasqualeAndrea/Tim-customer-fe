...
describe('T-15 Reachability form sci with login', () => {
  ...
  it('T-15 Reaches Yolo home page', () => {
    cy.visit(env.url)
    cy.wait(5000)
  ...
  it('T-15 Clicks ok on cookie banner', () => {
    cy.get('.cc-btn').click()
  })


  it('T-15 Reaches login page', () => {
    cy.get('.user-btn > .nav-link').click()
    cy.url().should('equal', env.url + '/login')
  })


  it('T-15 Compiles login form', () => {
