...
describe('T-2 Reachability form smartphone with login', () => {
  ...
  it('T-2 Clicks ok on cookie banner', () => {
    cy.get('.cc-btn').click()
  })


  it('T-2 Reaches login page', () => {
    cy.get('.user-btn > .nav-link').click()
    cy.url().should('equal', env.url + '/login')
  })


  it('T-2 Compiles login form', () => {
  ...
  it('T-13 Reaches Smartphone product', () => {
    cy.visit(env.url + '/preventivatore;code=cc-devices')
    cy.wait(5000)
  })


  it('T-2 Reaches form smartphone', () => {
    // cy.get('.nav > :nth-child(1) > .nav-link').click()
    // cy.get(':nth-child(2) > .flex-column > .go-to-arrow').click()
    cy.url().should('equal', env.url + '/preventivatore;code=cc-devices')
  })
