...
describe('T-25 Reachability form viaggio with login', () => {
  ...
  it('T-25 Reaches Yolo home page', () => {
    cy.visit(env.url)
    cy.wait(5000)
  ...
  it('T-25 Clicks ok on cookie banner', () => {
    cy.get('.cc-btn').click()
  })


  it('T-25 Reaches login page', () => {
    cy.get('.user-btn > .nav-link').click()
    cy.url().should('equal', env.url + '/login')
  })


  it('T-25 Compiles login form', () => {
  ...
  it('T-13 Reaches Viaggio product', () => {
    cy.visit(env.url + '/preventivatore;code=axa-assistance-gold,axa-assistance-silver')
    cy.wait(5000)
  })


  it('T-25 Reaches form viaggio', () => {
  //  cy.get('.nav > :nth-child(4) > .nav-link').click()
  //  cy.get(':nth-child(7) > .flex-column > .go-to-arrow').click()
    cy.url().should('include', env.url + '/preventivatore;code=axa-assistance-')
  })
