...
describe('T-01 MiFido checkout', () => {
    ...
    let mese
    let anno
    it('T-01 Visit preventivatore', () => {
        cy.visit(env.url + "/preventivatore;code=net-pet-gold,net-pet-silver")
    })

    it("T-01 Test quotator price", () => {
        cy.get("[data-cy=quotator-basic__quotation-price]").contains("11,00 €")
    })

    it("T-01 Click continua", () => {
        cy.get("[data-cy=quotator-basic__purchase-button]").first().click({ force: true })
    })

    it("T-02 Test click bottone ho un gatto", () => {
        cy.get("[data-cy=net-pet-checkout-info__select-cat-button]").click({ force: true })
        cy.get("[data-cy=net-pet-checkout-info__select-cat-image]").parent(".pet-choice").first().should('have.class', 'selected-pet')
        cy.get("[data-cy=net-pet-checkout-info__select-dog-image]").parent(".pet-choice").first().should('have.class', 'unselected-pet')
    })

    it("T-02 Test click bottone ho un cane", () => {
        cy.get("[data-cy=net-pet-checkout-info__select-dog-button]").click({ force: true })
        cy.get("[data-cy=net-pet-checkout-info__select-cat-image]").parent(".pet-choice").first().should('have.class', 'unselected-pet')
        cy.get("[data-cy=net-pet-checkout-info__select-dog-image]").parent(".pet-choice").first().should('have.class', 'selected-pet')
    })

    it("T-02 Test click immagine ho un gatto", () => {
        cy.get("[data-cy=net-pet-checkout-info__select-cat-image]").click({ force: true })
        cy.get("[data-cy=net-pet-checkout-info__select-cat-image]").parent(".pet-choice").first().should('have.class', 'selected-pet')
        cy.get("[data-cy=net-pet-checkout-info__select-dog-image]").parent(".pet-choice").first().should('have.class', 'unselected-pet')
    })

    it("T-02 Test click immagine ho un cane", () => {
        cy.get("[data-cy=net-pet-checkout-info__select-dog-image]").click({ force: true })
        cy.get("[data-cy=net-pet-checkout-info__select-cat-image]").parent(".pet-choice").first().should('have.class', 'unselected-pet')
        cy.get("[data-cy=net-pet-checkout-info__select-dog-image]").parent(".pet-choice").first().should('have.class', 'selected-pet')
    })

    it("T-02 Compilazione nome animale", () => {
        cy.get('[data-cy=net-pet-checkout-info__pet-name]').type("fuffi")
    })

    it("T-02 Compilazione data nascita animale", () => {
    ...
    it("T-02 Controllo card degli step non ancora completati", () => {
    ...
    it("T-02 Click continua", () => {
        cy.get('[data-cy=chekout-step-info__continue-button]').click({ force : true })
    })

    it("T-03 Controllo shopping-cart", () => {
        cy.get('app-checkout-cost-item-details-shopping-cart.ng-star-inserted > .row-shadowed > :nth-child(1)').should('be.visible')
        cy.get('[data-cy=checkout-cost-item-details__product-name]').contains('YOLO MiFido Gold')
        cy.get('[data-cy=checkout-cost-item-details__price]').contains('€11,00')
        var startdate = Cypress.moment().format('MM/YYYY')
        var enddate = Cypress.moment().add(1, 'month').format('MM/YYYY')
        //   cy.get('[data-cy=checkout-cost-item-details__validity-coverage]').contains(startdate+' - '+enddate)
        ...
        cy.get('[data-cy=checkout-cost-item-details__responsabilita-civile-per-danni-a-terzi-item]').contains('Responsabilità civile per danni a terzi')
        cy.get('[data-cy=checkout-cost-item-details__tutela-legale-item]').contains('Tutela legale')
    })

    it('T-03 Controllo card dati contraente', () => {
    ...
    it('T-03 Controllo card dati assicurazione', () => {
    ...
    it('T-03 Controllo step non completati', () => {
        cy.get('[data-cy=chekout-uncompleted-step__pagamento-card]').should('be.visible')
    })

    it('T-03 Test questionario 1 risposta NO', () => {
    ...
    it('T-03 Test questionario 2 risposta NO', () => {
    ...
    it('T-03 Test questionario 3 risposta NO', () => {
    ...
    it('T-03 Test questionario 1 risposta SI', () => {
    ...
    it('T-03 Test questionario 2 risposta SI', () => {
    ...
    it('T-03 Test questionario 3 risposta SI', () => {
    ...
    it('T-04 Check flag set informativo', () => {
        cy.get('.checkbox').click({ force: true })
    })

    it('T-04 Test card step precedenti non presenti', () => {
    ...
    it('T-04 Click bottone continua', () => {
        cy.get('#continuaStep4_test').click({ force: true })
    })

    it('T-05 Test h2 acquisto completato', () => {
