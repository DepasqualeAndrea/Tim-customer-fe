...
export class CheckoutGeSkiSeasonalContentService implements CheckoutContentProvider {
  ...
  private getContentFromKentico(): Observable<any> {
    return this.kenticoTranslateService.getItem('checkout_sci_stagionale_genertel').pipe
  ...
  createContentStructureFromKenticoItem(kenticoItem: any) {
    const structure = {
      ...
      cost_item: {
        ...
        info_set_ge_sport_plus: kenticoItem.shopping_cart___desktop.value.find(item => item.system.codename === 'info_set_ge_ski_seasonal_plus').text.value,
        info_set_ge_sport_premium: kenticoItem.shopping_cart___desktop.value.find(item => item.system.codename === 'info_set_ge_ski_seasonal_premium').text.value,
        ...
        coverage_options_splited: kenticoItem.coverage_options_splited.value,
        cost_detail_by_product: {
          geskiseasonalplus: kenticoItem.coverage_options_gold.value.map(item => item.text.value),
          geskiseasonalpremium: kenticoItem.coverage_options_silver.value.map(item => item.text.value)
        }
      },
    };
    return structure;
