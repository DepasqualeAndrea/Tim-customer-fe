...
export class CheckoutGeSportContentService implements CheckoutContentProvider {
  ...
  private getContentFromKentico(): Observable<any> {
    return this.kenticoTranslateService.getItem('checkout_sport_genertel').pipe
  ...
  createContentStructureFromKenticoItem(kenticoItem: any) {
    const structure = {
      ...
      card_insured: {
        ...
        package: kenticoItem.step_insurance_info.value.find(item => item.system.codename === 'package_title').text.value,
      ...
      cost_item: {
        ...
        info_set_ge_sport_plus: kenticoItem.shopping_cart___desktop.value.find(item => item.system.codename === 'info_set_ge_sport_plus').text.value,
        info_set_ge_sport_premium: kenticoItem.shopping_cart___desktop.value.find(item => item.system.codename === 'info_set_ge_sport_premium').text.value,
        ...
        cost_detail_by_product: {
          gesportplus: kenticoItem.coverage_options_gold.value.map(item => item.text.value),
          gesportpremium: kenticoItem.coverage_options_silver.value.map(item => item.text.value)
        }
      },
    };
    return structure;
