...
export class CheckoutIntesaSciContentService implements CheckoutContentProvider {
  ...
  createContentStructureFromKenticoItem(kenticoItem: any) {
    const structure = {
      ...
      cost_item: {
        ...
        informative_set_gold:   kenticoItem.shopping_cart_desktop.value.find(item => item.system.codename === 'set_informativo_sci_gold').text.value,
        informative_set_silver: kenticoItem.shopping_cart_desktop.value.find(item => item.system.codename === 'set_informativo_sci_silver').text.value,
        ...
        cost_detail_by_product: {
          ergomountaingold: kenticoItem.coverage_options_gold.value.map(item => item.text.value),
          ergomountainsilver: kenticoItem.coverage_options_silver.value.map(item => item.text.value)
        }
      },
    };
    return structure;
