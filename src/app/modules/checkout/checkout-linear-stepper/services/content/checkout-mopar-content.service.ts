...
@Injectable(
  ...
  export class CheckoutCouponTiresContentService implements CheckoutContentProvider {
      ...
      createContentStructureFromKenticoItem(kenticoItem: any) {
        ...
        const structure = {
          ...
          cost_item: {
            ...
            informative_set:     kenticoItem.shopping_cart_desktop.value.find(item => item.system.codename === 'informative_set_covered_tires').text.value,
            next_price:          kenticoItem.shopping_cart_desktop.value.find(item => item.system.codename === 'next_prize').text.value,
            zero_price:          kenticoItem.shopping_cart_desktop.value.find(item => item.system.codename === 'zero_price').text.value,
            shopping_free_month: kenticoItem.shopping_cart_desktop.value.find(item => item.system.codename === 'shopping_three_month_free').text.value,
            ...
            cost_detail_by_product: {
              coveatirescoveredhomage:   kenticoItem.coverage_options_standard.value.map(item => item.text.value)
          }
          },
          checkout_header: {
