...
export class CheckoutWinterSportContentService implements CheckoutContentProvider {
  ...
  createContentStructureFromKenticoItem(kenticoItem: any) {
    const structure = {
      checkout_header: {
        ...
        partner_icon_white:   kenticoItem.step_bar.value.find(item => item.system.codename === 'genertel_logo_white').thumbnail.value[0].url,
      ...
      cost_item: {
        ...
        cost_detail_by_product: {
          wintersportplus: kenticoItem.coverage_options_silver.value.map(item => item.text.value),
          wintersportpremium: kenticoItem.coverage_options_gold.value.map(item => item.text.value)
        }
      },
    };
    return structure;
