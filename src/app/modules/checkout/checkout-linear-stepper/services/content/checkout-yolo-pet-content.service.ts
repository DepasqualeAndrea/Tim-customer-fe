...
export class CheckoutYoloPetContentService implements CheckoutContentProvider {
  ...
  private getContentFromKentico(): Observable<any> {
    return this.kenticoTranslateService.getItem('checkout_yolo_pet').pipe
  ...
  createContentStructureFromKenticoItem(kenticoItem: any) {
    const structure = {
      ...
      cost_item: {
        ...
        informative_set: kenticoItem.shopping_cart_desktop.value.find(item => item.system.codename === 'information_package').text.value,
        ...
        cost_detail_by_product: {
          netpet: kenticoItem.coverage_options.value[0].linked_items.value.map(item => item.text.value),
        }
      }
    };
    return structure;
