...
import { CheckoutBaggageLossContent } from '../content/checkout-baggage-loss-content.model';
...
export class CheckoutLinearStepperBaggageLossLongTermReducer extends CheckoutLinearStepperGenericReducer implements CheckoutLinearStepperReducer {
  ...
  private setContent(state: CheckoutLinearStepperGenericState, content: CheckoutBaggageLossContent): CheckoutLinearStepperGenericState {
  ...
  private setContentHeader(state: CheckoutLinearStepperGenericState, content: CheckoutBaggageLossContent): CheckoutLinearStepperGenericState {
  ...
  private setCostItemDetailsContent(state: CheckoutLinearStepperGenericState, content: CheckoutBaggageLossContent): CheckoutLinearStepperGenericState {
  ...
  private setContentCompletedSteps(state: CheckoutLinearStepperGenericState, content: CheckoutBaggageLossContent) {
  ...
  private setUncompletedStepTitles(state: CheckoutLinearStepperGenericState, content: CheckoutBaggageLossContent) {
  ...
  private setContentInsuranceInfo(state: CheckoutLinearStepperGenericState, content: CheckoutBaggageLossContent): CheckoutLinearStepperGenericState {
    state.completed_steps.insurance_info.title = content.card_insured.title;
    state.completed_steps.insurance_info.display_data[0].label = content.card_insured.coverage_start_date;
    state.completed_steps.insurance_info.display_data[1].label = content.card_insured.coverage_expiration_date;
    state.completed_steps.insurance_info.display_data[2].label = content.card_insured.plate;
    return state;
  }

  private setContentInsuranceHolder(state: CheckoutLinearStepperGenericState, content: CheckoutBaggageLossContent): CheckoutLinearStepperGenericState {
  ...
  private setContentSurvey(state: CheckoutLinearStepperGenericState, content: CheckoutBaggageLossContent): CheckoutLinearStepperGenericState {
  ...
  private setInsured(state: CheckoutLinearStepperGenericState, order: ResponseOrder): CheckoutLinearStepperGenericState {
    ...
    displayData[0].value = CheckoutLinearStepperCommonReducer.convertDateAssuranceFormat(startDate);
    displayData[1].value = CheckoutLinearStepperCommonReducer.convertDateAssuranceFormat(expirationDate);
    displayData[2].value = lineItem.insurance_info.license_plate;
  ...
  public setCostItemDetail(state: CheckoutLinearStepperGenericState, order: ResponseOrder): CheckoutLinearStepperGenericState {
    ...
    const startDate = new Date(lineItem.start_date);
    const expirationDate = new Date(lineItem.expiration_date);
    state.cost_item.policy_startDate = CheckoutLinearStepperCommonReducer.convertDateAssuranceFormat(startDate);
    state.cost_item.policy_endDate = CheckoutLinearStepperCommonReducer.convertDateAssuranceFormat(expirationDate);
  ...
  private setCompletedStepsVisibility(state: CheckoutLinearStepperGenericState, completed_steps: [{ name: string, stepnum: number }]) {
    ...
    // const isLoginRegister = !!this.state.currentUrl && this.state.currentUrl.includes('login-register');
    // if (this.state.model.currentStep.stepnum === 1 && isLoginRegister) {
    //   state.completed_steps.insurance_info.visible = true;
    // }
    // //const uncompleted_steps_filtered = state.uncompleted_steps.filter( x => x.conw)
    state.completed_steps = Object.assign({}, state.completed_steps);
    return state;
  }
