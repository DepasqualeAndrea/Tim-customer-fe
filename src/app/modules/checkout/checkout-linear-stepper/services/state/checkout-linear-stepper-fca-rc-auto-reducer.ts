...
import { CheckoutFcaRcAutoContent } from '../content/checkout-fca-rc-auto-content.model';
import * as moment from 'moment';


export class CheckoutLinearStepperFcaRcAutoReducer extends CheckoutLinearStepperGenericReducer
    ...
    public reduce(actionName: string, payload: any) {
        switch (actionName) {
            ...
            case 'setResponseOrder':
                ...
                break;
            case 'setVehicle':
                this.state = this.setVehicle(this.state, payload);
                break;
            case 'setContractor':
    ...
    private setContent(state: CheckoutLinearStepperGenericState
        , content: CheckoutFcaRcAutoContent): CheckoutLinearStepperGenericState {
    ...
    private setContentCompletedSteps(state: CheckoutLinearStepperGenericState, content: CheckoutFcaRcAutoContent) {
    ...
    private setUncompletedStepTitles(state: CheckoutLinearStepperGenericState, content: CheckoutFcaRcAutoContent) {
        state.uncompleted_step_titles = [
          {
            name: 'insurance-info',
            title: content.card_vehicle.title
          },
          {
            name: 'address',
          ...
          {
            name: 'survey',
            title: content.card_consents.title
          },
          {
            name: 'payment',
    ...
    private setContentInsuranceInfo(state: CheckoutLinearStepperGenericState, content: CheckoutFcaRcAutoContent): CheckoutLinearStepperGenericState {
        state.completed_steps.insurance_info.title = content.card_vehicle.title;
        state.completed_steps.insurance_info.display_data[0].label = content.card_vehicle.displacement;
        state.completed_steps.insurance_info.display_data[1].label = content.card_vehicle.birth_date;
        state.completed_steps.insurance_info.display_data[2].label = content.card_vehicle.residential_city;
        return state;
    }
    private setContentInsuranceHolder(state: CheckoutLinearStepperGenericState, content: CheckoutFcaRcAutoContent): CheckoutLinearStepperGenericState {
    ...
    private setContentSurvey(state: CheckoutLinearStepperGenericState, content: CheckoutFcaRcAutoContent): CheckoutLinearStepperGenericState {
        state.completed_steps.insurance_survey.title = content.card_consents.title;
        state.completed_steps.insurance_survey.card_icon = content.card_consents.image;
        state.completed_steps.insurance_survey.state = content.card_consents.status;
        state.completed_steps.insurance_survey.description = content.card_consents.description;
        return state;
    }
    ...
    private setCompletedSteps(state: CheckoutLinearStepperGenericState, order: ResponseOrder) {
        state = this.setVehicle(state, order);
    ...
    private setVehicle(state: CheckoutLinearStepperGenericState, order: ResponseOrder): CheckoutLinearStepperGenericState {
            const lineItem = order.line_items[0];
            const displayData = state.completed_steps.insurance_info.display_data;
            const insuredBirthDate = lineItem.insurance_info.birth_date
            displayData[0].value = lineItem.insurance_info.displacement_label;
            displayData[1].value = moment(insuredBirthDate).format('DD/MM/YYYY')
            displayData[2].value = lineItem.insurance_info.province;
