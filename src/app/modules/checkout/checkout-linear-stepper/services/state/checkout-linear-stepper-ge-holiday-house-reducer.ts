...
import { CheckoutGeHolidayHouseContent } from '../content/checkout-ge-holiday-house-content.model';



export class CheckoutLinearStepperGeHolidayHouseReducer extends CheckoutLinearStepperGenericReducer
  ...
  public setCostItemDetail(state: CheckoutLinearStepperGenericState, order: ResponseOrder): CheckoutLinearStepperGenericState {
    this.state.model.product.code === 'ge-holiday-house-plus'
    ? this.state.cost_item.informative_set = state.cost_item.informative_set_plus
    : this.state.cost_item.informative_set = state.cost_item.informative_set_premium;
    if (order.state === 'complete'
     && !this.state.model.product.code.includes('ge-holiday-house')
    ) {
  ...
  private setContent(state: CheckoutLinearStepperGenericState
    , content: CheckoutGeHolidayHouseContent): CheckoutLinearStepperGenericState {
  ...
  private setContentCompletedSteps(state: CheckoutLinearStepperGenericState,
                                   content: CheckoutGeHolidayHouseContent) {
  ...
  private setUncompletedStepTitles(state: CheckoutLinearStepperGenericState, content: CheckoutGeHolidayHouseContent) {
    state.uncompleted_step_titles = [
      {
        name: 'insurance-info',
        title: content.card_house.title
      },
      {
        name: 'address',
  ...
  private setContentInsuranceInfo(state: CheckoutLinearStepperGenericState,
                                  content: CheckoutGeHolidayHouseContent): CheckoutLinearStepperGenericState {
    state.completed_steps.insurance_info.title = content.card_insurance.title;

    state.completed_steps.insurance_info.kentico_insurance_info.duration_insurance = content.card_house.duration_insurance;
    state.completed_steps.insurance_info.kentico_insurance_info.package = content.card_house.package;
    state.completed_steps.insurance_info.kentico_insurance_info.insured = content.card_house.insured;
  ...
  private setBike(state: CheckoutLinearStepperGenericState, order: ResponseOrder): CheckoutLinearStepperGenericState {
    ...
    const lineItem = order.line_items[0];
        ...
        displayData[0] = {label: kenticoInsuranceInfo.duration_insurance, value: this.setDuration(order)};
        displayData[1] = {label: kenticoInsuranceInfo.package, value: lineItem.variant.name};
        displayData[2] = {label: kenticoInsuranceInfo.insured, value: lineItem.quantity};
    return state;
  }
