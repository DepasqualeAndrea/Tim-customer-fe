...
export class CheckoutLinearStepperGeSkiSeasonalReducer extends CheckoutLinearStepperGenericReducer implements CheckoutLinearStepperReducer {
  ...
  private mapCovarageOptions(state) {
    return state.cost_item.coverage_options_splited.map(item => {
      const coverage_titles = [];
      const coverage_subtitles = item.subitems.value.map(i => i.text.value);
      coverage_titles.push(item.text.value);
      const obj = {
        title: item.text.value,
        subtitle: coverage_subtitles
      };
      return obj;
    });
  }
  ...
  private setCostItemDetailsContent(state: CheckoutLinearStepperGenericState, content: CheckoutGeSkiContent): CheckoutLinearStepperGenericState {
    ...
    state.cost_item.const_detail_coverage_options = this.mapCovarageOptions(state);
    return state;
  }
  ...
  private setInsured(state: CheckoutLinearStepperGenericState, order: ResponseOrder): CheckoutLinearStepperGenericState {
    ...
    displayData[1].value = lineItem.variant.name;
  ...
  public setCostItemDetail(state: CheckoutLinearStepperGenericState, order: ResponseOrder): CheckoutLinearStepperGenericState {
    this.state.model.product.code === 'ge-ski-seasonal-plus'
    ...
    if (order.line_items[0].addons.length > 0 && !state.model.product.detailProductDeleted) {
      ...
      const coverages = state.cost_item.const_detail_coverage_options;
      addons.map(item => {
        switch (item.description) {
          case 'Protezione del patrimonio':
          coverages[2].subtitle.push(item.name);
          break;
          case 'Cura della persona':
          coverages[1].subtitle.push(item.name);
          break;
        }
      });
      state.model.product.detailProductDeleted = state.cost_item.const_detail_coverage_options;
    }
