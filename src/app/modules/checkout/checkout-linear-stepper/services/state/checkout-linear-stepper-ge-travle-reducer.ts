...
import { CheckoutGeTravelContent } from '../content/checkout-ge-travel-content.model';

export class CheckoutLinearStepperGeTravelReducer extends CheckoutLinearStepperGenericReducer implements CheckoutLinearStepperReducer {
  ...
  private setContent(state: CheckoutLinearStepperGenericState, content: CheckoutGeTravelContent): CheckoutLinearStepperGenericState {
  ...
  private setContentHeader(state: CheckoutLinearStepperGenericState, content: CheckoutGeTravelContent): CheckoutLinearStepperGenericState {
  ...
  private setCostItemDetailsContent(state: CheckoutLinearStepperGenericState, content: CheckoutGeTravelContent): CheckoutLinearStepperGenericState {
  ...
  private setContentCompletedSteps(state: CheckoutLinearStepperGenericState, content: CheckoutGeTravelContent) {
  ...
  private setUncompletedStepTitles(state: CheckoutLinearStepperGenericState, content: CheckoutGeTravelContent) {
  ...
  private setContentInsuranceInfo(state: CheckoutLinearStepperGenericState, content: CheckoutGeTravelContent): CheckoutLinearStepperGenericState {
    state.completed_steps.insurance_info.title = content.card_insured.title;
    state.completed_steps.insurance_info.display_data[0].label = content.card_insured.main_destination;
    ...
    state.completed_steps.insurance_info.display_data[4].label = content.card_insured.end_date;


    return state;
  }

  private setContentInsuranceHolder(state: CheckoutLinearStepperGenericState, content: CheckoutGeTravelContent): CheckoutLinearStepperGenericState {
  ...
  private setContentSurvey(state: CheckoutLinearStepperGenericState, content: CheckoutGeTravelContent): CheckoutLinearStepperGenericState {
  ...
  private setInsured(state: CheckoutLinearStepperGenericState, order: ResponseOrder): CheckoutLinearStepperGenericState {
    ...
    displayData[4].value = moment(lineItem.expiration_date).subtract(1, 'days').format('DD-MM-YYYY');
  ...
  public setCostItemDetail(state: CheckoutLinearStepperGenericState, order: ResponseOrder): CheckoutLinearStepperGenericState {
    this.state.model.product.code === 'ge-travel-plus'
    ? this.state.cost_item.informative_set = state.cost_item.info_set_ge_travel_plus
    : this.state.cost_item.informative_set = state.cost_item.info_set_ge_travel_premium;
