...
import { CheckoutPaiContent } from '../content/checkout-pai-content.model';
import { CheckoutLinearStepperCommonReducer } from './checkout-linear-stepper-common-reducer';
import { ResponseOrder } from '@model';

export class CheckoutLinearStepperPaiReducer extends CheckoutLinearStepperGenericReducer implements CheckoutLinearStepperReducer {
  ...
  public reduce(actionName: string, payload: any) {
    switch (actionName) {
      ...
      case 'setResponseOrder':
        ...
        break;
      case 'setDrivers':
        this.state = this.setDrivers(this.state, payload);
        break;
      case 'setContractor':
  ...
  private setContent(state: CheckoutLinearStepperGenericState, content: CheckoutPaiContent): CheckoutLinearStepperGenericState {
  ...
  private setContentHeader(state: CheckoutLinearStepperGenericState, content: CheckoutPaiContent): CheckoutLinearStepperGenericState {
  ...
  private setCostItemDetailsContent(state: CheckoutLinearStepperGenericState, content: CheckoutPaiContent): CheckoutLinearStepperGenericState {
  ...
  private setContentCompletedSteps(state: CheckoutLinearStepperGenericState, content: CheckoutPaiContent) {
  ...
  private setUncompletedStepTitles(state: CheckoutLinearStepperGenericState, content: CheckoutPaiContent) {
  ...
  private setContentInsuranceInfo(state: CheckoutLinearStepperGenericState, content: CheckoutPaiContent): CheckoutLinearStepperGenericState {
    ...
    state.completed_steps.insurance_info.display_data[2].label = content.card_insured.number_of_drivers;
    return state;
  }

  private setContentInsuranceHolder(state: CheckoutLinearStepperGenericState, content: CheckoutPaiContent): CheckoutLinearStepperGenericState {
  ...
  private setContentSurvey(state: CheckoutLinearStepperGenericState, content: CheckoutPaiContent): CheckoutLinearStepperGenericState {
  ...
  private setCompletedSteps(state: CheckoutLinearStepperGenericState, order: ResponseOrder) {
    state = this.setHeader(state, order);
    state = this.setDrivers(state, order);
  ...
  private setDrivers(state: CheckoutLinearStepperGenericState, order: ResponseOrder): CheckoutLinearStepperGenericState {
    ...
    displayData[0].value = CheckoutLinearStepperCommonReducer.convertDateFormat(startDate);
    displayData[1].value = CheckoutLinearStepperCommonReducer.convertDateFormat(expirationDate);
