...
import { CheckoutMoparContent } from '../content/checkout-mopar-content.model';
import { CheckoutCouponTiresContentService } from '../content/checkout-mopar-content.service';

export class CheckoutLinearStepperPromoTiresReducer extends CheckoutLinearStepperGenericReducer
    ...
    private setContent(state: CheckoutLinearStepperGenericState
        , content: CheckoutMoparContent): CheckoutLinearStepperGenericState {
    ...
    private setContentCompletedSteps(state: CheckoutLinearStepperGenericState, content: CheckoutMoparContent) {
    ...
    private setUncompletedStepTitles(state: CheckoutLinearStepperGenericState, content: CheckoutMoparContent) {
    ...
    private setContentInsuranceInfo(state: CheckoutLinearStepperGenericState, content: CheckoutMoparContent): CheckoutLinearStepperGenericState {
        state.completed_steps.insurance_info.title = content.card_vehicle.title;
        state.completed_steps.insurance_info.display_data[0].label = content.card_vehicle.coverage_start_date;
        state.completed_steps.insurance_info.display_data[1].label = content.card_vehicle.coverage_expiration_date;
        state.completed_steps.insurance_info.display_data[2].label = content.card_vehicle.license_plate;
        return state;
    }
    private setContentInsuranceHolder(state: CheckoutLinearStepperGenericState, content: CheckoutMoparContent): CheckoutLinearStepperGenericState {
    ...
    private setContentSurvey(state: CheckoutLinearStepperGenericState, content: CheckoutMoparContent): CheckoutLinearStepperGenericState {
    ...
    private setInsured(state: CheckoutLinearStepperGenericState, order: ResponseOrder): CheckoutLinearStepperGenericState {
        ...
        if (!lineItem.vehicle || !lineItem.vehicle.license_plate) {
            state.completed_steps.insurance_info.visible = false;
            return state;
        }
        if (lineItem.vehicle && lineItem.vehicle.license_plate) {
            const licensePlate = lineItem.vehicle.license_plate;
            if (!licensePlate) {
                state.completed_steps.insurance_info.visible = false;
                return state;
            }
            displayData[2].value = licensePlate;
        }
        return state;
    ...
    private showInsuranceInfoWhenLoginRegister(state: CheckoutLinearStepperGenericState, payload: { url: string }): CheckoutLinearStepperGenericState {
        ...
        if (isLoginRegister && this.state.model.currentStep.stepnum === 1) {
            state.completed_steps.insurance_info.visible = true;
            state.completed_steps = Object.assign({}, state.completed_steps);
            this.setInsuredFromProduct(state);
    ...
    private setInsuredFromProduct(state: CheckoutLinearStepperGenericState) {
        const displayData = state.completed_steps.insurance_info.display_data;
        const licensePlate = (state.model.product as any).licensePlate;
        displayData[2].value = licensePlate;
        return state;
    }
}
