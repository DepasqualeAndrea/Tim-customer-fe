...
import { CheckoutScreenProtectionContent } from '../content/checkout-screen-protection-content-model';

export class CheckoutLinearStepperScreenProtectionReducer extends CheckoutLinearStepperGenericReducer implements CheckoutLinearStepperReducer {
  ...
  private setContent(state: CheckoutLinearStepperGenericState, content: CheckoutScreenProtectionContent): CheckoutLinearStepperGenericState {
  ...
  private setContentHeader(state: CheckoutLinearStepperGenericState, content: CheckoutScreenProtectionContent): CheckoutLinearStepperGenericState {
  ...
  private setCostItemDetailsContent(state: CheckoutLinearStepperGenericState, content: CheckoutScreenProtectionContent): CheckoutLinearStepperGenericState {
  ...
  private setContentCompletedSteps(state: CheckoutLinearStepperGenericState, content: CheckoutScreenProtectionContent) {
  ...
  private setContentInsuranceInfo(state: CheckoutLinearStepperGenericState, content: CheckoutScreenProtectionContent): CheckoutLinearStepperGenericState {
    state.completed_steps.insurance_info.title = content.card_screen_protection.title;
    state.completed_steps.insurance_info.display_data[0].label = content.card_screen_protection.package;
    state.completed_steps.insurance_info.display_data[1].label = content.card_screen_protection.insured_number;
    return state;
  }

  private setUncompletedStepTitles(state: CheckoutLinearStepperGenericState, content: CheckoutScreenProtectionContent) {
  ...
  private setContentInsuranceHolder(state: CheckoutLinearStepperGenericState, content: CheckoutScreenProtectionContent): CheckoutLinearStepperGenericState {
  ...
  private setContentSurvey(state: CheckoutLinearStepperGenericState, content: CheckoutScreenProtectionContent): CheckoutLinearStepperGenericState {
  ...
  private setCompletedSteps(state: CheckoutLinearStepperGenericState, order: ResponseOrder) {
    ...
    state = this.setScreenProtection(state, order);
    state.completed_steps = Object.assign({}, state.completed_steps);
    return state;
  }

  private setScreenProtection(state: CheckoutLinearStepperGenericState, order: ResponseOrder): CheckoutLinearStepperGenericState {
    ...
    const productVariantName = lineItem.variant.name;
    displayData[0].value = productVariantName;
    displayData[1].value = lineItem.insurance_info.covercare_imei;
    return state;
  }
  ...
  private setCompletedStepsVisibility(state: CheckoutLinearStepperGenericState, completed_steps: [{ name: string, stepnum: number }]) {
    ...
    if(this.state.model.currentStep.name === 'payment'){
