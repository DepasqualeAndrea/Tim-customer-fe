...
import { CheckoutTimMyHomeContent } from '../content/checkout-tim-my-home-content.model';
import _ from 'lodash';
import { DataService } from '@services';

export class CheckoutLinearStepperTimMyHomeReducer extends CheckoutLinearStepperGenericReducer implements CheckoutLinearStepperReducer {
  ...
  public reduce(actionName: string, payload: any) {
    switch (actionName) {
      ...
      case 'setCostItemAfterSelectedAddonsHome':
        ...
        DataService.TIM_MY_HOME_ADDONS = payload.addons;
        break;
      case 'setOrderChange':
      ...
      case 'setProperty':
        ...
        break;
      case 'proposalTitle':
        this.state = this.updateProposalTitle(this.state, payload);
        break;
      /*  case 'proposalBack':
          this.state = this.updateProposalTitleBack(this.state, payload);*/
    }
  ...
  public setCostItemDetail(state: CheckoutLinearStepperGenericState, order: ResponseOrder): CheckoutLinearStepperGenericState {
    state.cost_item.cost_detail_by_product = {
      timmyhome: [state.cost_item.default_addon]
    };
    ...
    if (order.state === 'complete' && !window.matchMedia('(min-width: 576px)').matches) {
      localStorage.setItem('shoppingCartContent', JSON.stringify(state.cost_item));
      state.cost_item = null;
      return state;
    }
    if (order.line_items[0].payment_frequency === 'M') {
      state.cost_item.price_frequency_annual = (order.currency === 'EUR' ? '€' : null) + (order.total * 12);
    }
  ...
  private setContent(state: CheckoutLinearStepperGenericState, content: CheckoutTimMyHomeContent): CheckoutLinearStepperGenericState {
  ...
  private setContentHeader(state: CheckoutLinearStepperGenericState, content: CheckoutTimMyHomeContent): CheckoutLinearStepperGenericState {
  ...
  private setCostItemDetailsContent(state: CheckoutLinearStepperGenericState, content: CheckoutTimMyHomeContent): CheckoutLinearStepperGenericState {
  ...
  private setCostItemListAddons(state: CheckoutLinearStepperGenericState, addons: any) {
    ...
    state.cost_item.cost_detail_with_code_by_product.timmyhome.forEach(item => {
      const isSelected = (addon) => addon.selected;
      addons.filter(isSelected).forEach(addon => {
        if (addon.id === item.code.value) {
          selectedAddons.push(item.name.value);
          return;
        }
      });
    });
    ...
    state.cost_item.cost_detail_by_product = {
      timmyhome: selectedAddons
    };
    return state;
  }

  private setContentCompletedSteps(state: CheckoutLinearStepperGenericState, content: CheckoutTimMyHomeContent) {
  ...
  private setUncompletedStepTitles(state: CheckoutLinearStepperGenericState, content: CheckoutTimMyHomeContent) {
  ...
  private setContentInsuranceInfo(state: CheckoutLinearStepperGenericState, content: CheckoutTimMyHomeContent): CheckoutLinearStepperGenericState {
  ...
  private setContentInsuranceHolder(state: CheckoutLinearStepperGenericState, content: CheckoutTimMyHomeContent): CheckoutLinearStepperGenericState {
  ...
  private setContentSurvey(state: CheckoutLinearStepperGenericState, content: CheckoutTimMyHomeContent): CheckoutLinearStepperGenericState {
  ...
  private setCompletedStepsVisibility(state: CheckoutLinearStepperGenericState, completed_steps: [{ name: string, stepnum: number }]) {
    ...
    state.completed_steps.insurance_survey.visible = false;
    state.completed_steps.insurance_holder.visible = false;
    state.completed_steps.insurance_info.addons_recap = false;
    if (this.state.model.currentStep.name !== 'complete') {
      completed_steps.forEach(step => {
        switch (step.name) {
          case 'insurance-info':
            state.completed_steps.insurance_info.visible = false;
            state.completed_steps.insurance_info.addons_recap = true;
  ...
  private updateCostItem(state: CheckoutLinearStepperGenericState, payload: any) {
    const addons = payload.addons || payload.data?.addons || DataService.TIM_MY_HOME_ADDONS;
    if (addons?.length > 0) {
    ...
    if (state.cost_item.price_frequency_annual) {
  ...
  private updateProposalTitle(state: CheckoutLinearStepperGenericState, payload: any) {
    if (payload === 'basic') {
      state.cost_item.informative_set = '';
      state.cost_item.informative_set_double = '';
    }
    return state;
  }
}
