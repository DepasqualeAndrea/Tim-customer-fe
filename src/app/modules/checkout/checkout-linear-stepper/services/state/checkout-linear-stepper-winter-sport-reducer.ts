...
import { CheckoutGeSkiContent } from '../content/checkout-ge-ski-content.model';

export class CheckoutLinearStepperWinterSportReducer extends CheckoutLinearStepperGenericReducer implements CheckoutLinearStepperReducer {
  ...
  private getEmptyState(): CheckoutLinearStepperGenericState {
    const initialState = {
      checkout_header: {
        ...
        partner_name: null,
        partner_icon: null,
        assicurazione_sci_plus: null,
        assicurazione_sci_premium: null,
      },
      completed_steps: {
        insurance_info: {
          visible: false,
  ...
  private setContent(state: CheckoutLinearStepperGenericState, content: CheckoutGeSkiContent): CheckoutLinearStepperGenericState {
  ...
  private setContentHeader(state: CheckoutLinearStepperGenericState, content: CheckoutGeSkiContent): CheckoutLinearStepperGenericState {
    ...
    state.checkout_header.partner_icon = content.checkout_header.partner_icon_white;
    state.checkout_header.assicurazione_sci_plus =  content.checkout_header.assicurazione_sci_plus;
    state.checkout_header.assicurazione_sci_premium =  content.checkout_header.assicurazione_sci_premium;
    return state;
  }

  private setCostItemDetailsContent(state: CheckoutLinearStepperGenericState, content: CheckoutGeSkiContent): CheckoutLinearStepperGenericState {
  ...
  private setContentCompletedSteps(state: CheckoutLinearStepperGenericState, content: CheckoutGeSkiContent) {
  ...
  private setUncompletedStepTitles(state: CheckoutLinearStepperGenericState, content: CheckoutGeSkiContent) {
  ...
  private setContentInsuranceInfo(state: CheckoutLinearStepperGenericState, content: CheckoutGeSkiContent): CheckoutLinearStepperGenericState {
    state.completed_steps.insurance_info.title = content.card_insured.title;
    state.completed_steps.insurance_info.display_data[0].label = content.card_insured.duration;
    state.completed_steps.insurance_info.display_data[2].label = content.card_insured.insured;
    state.completed_steps.insurance_info.display_data[3].label = content.card_insured.where_play_sport;
    state.completed_steps.insurance_info.subtitle = content.card_insured.protezione_sport_invernali;
    return state;
  }

  private setContentInsuranceHolder(state: CheckoutLinearStepperGenericState, content: CheckoutGeSkiContent): CheckoutLinearStepperGenericState {
  ...
  private setContentSurvey(state: CheckoutLinearStepperGenericState, content: CheckoutGeSkiContent): CheckoutLinearStepperGenericState {
  ...
  private setHeader(state: CheckoutLinearStepperGenericState, order: ResponseOrder): CheckoutLinearStepperGenericState {
    ...
    state.checkout_header.product_name = state.model.product.code === 'winter-sport-plus' ?  state.checkout_header.assicurazione_sci_plus : state.checkout_header.assicurazione_sci_premium;
  ...
  private setInsured(state: CheckoutLinearStepperGenericState, order: ResponseOrder): CheckoutLinearStepperGenericState {
    ...
    displayData[0].value = this.setDuration(order);
    displayData[2].value = lineItem.quantity;
    displayData[3].value = lineItem.insurance_info.travel_destination;
  ...
  public setCostItemDetail(state: CheckoutLinearStepperGenericState, order: ResponseOrder): CheckoutLinearStepperGenericState {
    ...
    this.state.model.product.code === 'winter-sport-plus'
    ? this.state.cost_item.informative_set = state.cost_item.info_set_ge_sport_plus
    : this.state.cost_item.informative_set = state.cost_item.info_set_ge_sport_premium;
    CheckoutLinearStepperCommonReducer.setCostItemDetail(state, order);
    state.cost_item.product_name = state.model && this.state.model.product.code === 'winter-sport-plus'
    ? this.state.cost_item.product_name = state.cost_item.protezione_sport_invernali_plus
    : this.state.cost_item.product_name = state.cost_item.protezione_sport_invernali_premium;

    state.model && this.state.model.product.code === 'winter-sport-plus'
    ? this.state.cost_item.package = state.cost_item.assicurazione_sci_plus_1
    : this.state.cost_item.package = state.cost_item.assicurazione_sci_premium_2;

    order.line_items[0]?.variant?.sku === 'winter_sport_plus_seasonal' || order.line_items[0]?.variant?.sku === 'winter_sport_premium_seasonal'
    ? this.state.cost_item.isSeasonal = true : this.state.cost_item.isSeasonal = false;
    ...
    if (order.line_items[0].addons.length > 0 && !state.model.product.detailProductDeleted) {
      ...
      addons.map( item => {
        ...
        state.model.product.detailProductDeleted = state.cost_item.cost_detail_by_product[code].push(item.name);
      });
    }
    return state;
  }
