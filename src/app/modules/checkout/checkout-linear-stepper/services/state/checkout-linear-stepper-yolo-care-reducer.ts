import { CheckoutYoloCareContent } from '../content/checkout-yolo-care-content.model';
...
export class CheckoutLinearStepperYoloCareReducer extends CheckoutLinearStepperGenericReducer implements CheckoutLinearStepperReducer {
  ...
  private setContent(state: CheckoutLinearStepperGenericState, content: CheckoutYoloCareContent): CheckoutLinearStepperGenericState {
  ...
  private setContentHeader(state: CheckoutLinearStepperGenericState, content: CheckoutYoloCareContent): CheckoutLinearStepperGenericState {
  ...
  private setCostItemDetailsContent(state: CheckoutLinearStepperGenericState, content: CheckoutYoloCareContent): CheckoutLinearStepperGenericState {
    ...
    if (this.state.model && this.state.model.product) {
      ...
      state.cost_item.cost_detail_list = CheckoutLinearStepperCommonReducer.getCostItemDetailsByVariant(variantSku, state.cost_item.cost_detail_by_variant);
    }
    return state;
}

  private setContentCompletedSteps(state: CheckoutLinearStepperGenericState, content: CheckoutYoloCareContent) {
  ...
  private setUncompletedStepTitles(state: CheckoutLinearStepperGenericState, content: CheckoutYoloCareContent) {
  ...
  private setContentInsuranceInfo(state: CheckoutLinearStepperGenericState, content: CheckoutYoloCareContent): CheckoutLinearStepperGenericState {
    state.completed_steps.insurance_info.title = content.card_insured.title;
    state.completed_steps.insurance_info.display_data[0].label = content.card_insured.coverage_type;
    state.completed_steps.insurance_info.display_data[1].label = content.card_insured.insured_subjects;
    return state;
  }

  private setContentInsuranceHolder(state: CheckoutLinearStepperGenericState, content: CheckoutYoloCareContent): CheckoutLinearStepperGenericState {
  ...
  private setContentSurvey(state: CheckoutLinearStepperGenericState, content: CheckoutYoloCareContent): CheckoutLinearStepperGenericState {
  ...
  private setOrder(state: CheckoutLinearStepperGenericState, order: ResponseOrder): CheckoutLinearStepperGenericState {
    state = this.setVariantsNames(state, order);
  ...
  private setInsured(state: CheckoutLinearStepperGenericState, order: ResponseOrder): CheckoutLinearStepperGenericState {
    ...
    displayData[0].value = lineItem.variant.name;
    displayData[1].value = lineItem.quantity;
    state.completed_steps.insurance_info.product_icon = state.model.product.image;
    return state;
  }

  private setVariantsNames(state: CheckoutLinearStepperGenericState, order: ResponseOrder) {
    const product = state.model.product.originalProduct;
    const variants = product.variants.map((variant, index) => {
      variant.name = product.variants_names[index].name;
      return variant;
    });
    const lineItem = order.line_items[0];
    lineItem.variant.name = variants.find(v => v.id === lineItem.variant.id).name;
    return state;
  }
