...
export class CheckoutLinearStepperYoloForPetReducer extends CheckoutLinearStepperGenericReducer implements CheckoutLinearStepperReducer {
  ...
  private setContentInsuranceInfo(state: CheckoutLinearStepperGenericState, content: CheckoutPetContent): CheckoutLinearStepperGenericState {
    ...
    if (selectedPetType) {
      state.completed_steps.insurance_info.display_data[0].value = selectedPetType === 'dog'
        ? state.completed_steps.insurance_info.pet_type_dog
