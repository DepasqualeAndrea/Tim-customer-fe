...
import { CheckoutYoloViaggioContent } from '../content/checkout-yolo-viaggio-content.model';

export class CheckoutLinearStepperYoloViaggioReducer extends CheckoutLinearStepperGenericReducer implements CheckoutLinearStepperReducer {
  ...
  private setContent(state: CheckoutLinearStepperGenericState, content: CheckoutYoloViaggioContent): CheckoutLinearStepperGenericState {
  ...
  private setContentCollaborationSection(state: CheckoutLinearStepperGenericState, content: CheckoutYoloViaggioContent): CheckoutLinearStepperGenericState {
  ...
  private setContentHeader(state: CheckoutLinearStepperGenericState, content: CheckoutYoloViaggioContent): CheckoutLinearStepperGenericState {
  ...
  private setContentCompletedSteps(state: CheckoutLinearStepperGenericState, content: CheckoutYoloViaggioContent) {
  ...
  private setUncompletedStepTitles(state: CheckoutLinearStepperGenericState, content: CheckoutYoloViaggioContent) {
  ...
  private setContentInsuranceInfo(state: CheckoutLinearStepperGenericState, content: CheckoutYoloViaggioContent): CheckoutLinearStepperGenericState {
    state.completed_steps.insurance_info.title = content.card_insured.title;
    state.completed_steps.insurance_info.display_data[1].label= content.card_insured.package;
    state.completed_steps.insurance_info.duration= content.card_insured.duration;
    state.completed_steps.insurance_info.display_data[2].label = content.card_insured.insured_number;
    state.completed_steps.insurance_info.period = content.card_insured.period;
    state.completed_steps.insurance_info.display_data[3].label = content.card_insured.start_date;
    state.completed_steps.insurance_info.display_data[4].label = content.card_insured.expiration_date;
    state.completed_steps.insurance_info.people = content.card_insured.people;
    return state;
  }

  private setContentInsuranceHolder(state: CheckoutLinearStepperGenericState, content: CheckoutYoloViaggioContent): CheckoutLinearStepperGenericState {
  ...
  private setContentSurvey(state: CheckoutLinearStepperGenericState, content: CheckoutYoloViaggioContent): CheckoutLinearStepperGenericState {
  ...
  private setInsured(state: CheckoutLinearStepperGenericState, order: ResponseOrder): CheckoutLinearStepperGenericState {
    ...
    displayData[0].value = lineItem.insurance_info.destination.name;
    displayData[1].value = lineItem.variant.product_properties.uniq_name +" "+ lineItem.variant.product_properties.certificate.charAt(0).toUpperCase()+ lineItem.variant.product_properties.certificate.slice(1).toLowerCase();
    displayData[2].value = lineItem.quantity;
    displayData[3].value = moment(lineItem.start_date).format('DD-MM-YYYY');
    displayData[4].value = moment(lineItem.expiration_date).format('DD-MM-YYYY');
  ...
  public setCostItemDetail(state: CheckoutLinearStepperGenericState, order: ResponseOrder): CheckoutLinearStepperGenericState {
    state.cost_item.informative_set =
    state.model && state.model.product.code === 'axa-assistance-gold'
    ? state.cost_item.informative_set_gold : state.cost_item.informative_set_silver;
    ...
    state.cost_item.product_name = state.model && state.model.product.code === 'axa-assistance-silver'
      ? state.cost_item.yolo_viaggio_silver : state.cost_item.yolo_viaggio_gold;
    if (order.state === 'complete') {
  ...
  public setCostItemDetailsContent(state: CheckoutLinearStepperGenericState, content: CheckoutYoloViaggioContent): CheckoutLinearStepperGenericState {
  ...
  private setCostItemListCoverages(state: CheckoutLinearStepperGenericState, variant: any) {
    const coverageVariant = state.cost_item.cost_detail_by_product.coveragegoldsilver;
    if (variant.product_properties.alias === 'viaggio-silver') {
