...
export class CheckoutLinearStepperSportNewReducer extends CheckoutLinearStepperGenericReducer implements CheckoutLinearStepperReducer {
  ...
  private setContentInsuranceInfo(state: CheckoutLinearStepperGenericState, content: CheckoutSportContent): CheckoutLinearStepperGenericState {
    ...
    state.completed_steps.insurance_info.display_data[0].label = content.card_insured.sport;
    state.completed_steps.insurance_info.display_data[1].label = content.card_insured.number_of_insureds;
    state.completed_steps.insurance_info.display_data[2].label = content.card_insured.duration;
    state.completed_steps.insurance_info.subjectsDropdownTitle = content.card_insured.people;
    return state;
  }
  ...
  private setInsured(state: CheckoutLinearStepperGenericState, order: ResponseOrder): CheckoutLinearStepperGenericState {
    ...
    displayData[0].value = lineItem.insurance_info.extra;
    displayData[1].value = lineItem.quantity;
    displayData[2].value = lineItem.variant.option_values[0].presentation;
    state.completed_steps.insurance_info.product_icon = state.model.product.image;

    state.completed_steps.insurance_info.subjectsDropdown = true;
    state.completed_steps.insurance_info.insuredSubjectsDropdown = lineItem.insured_entities.insurance_holders.map(holder => 
      ({
