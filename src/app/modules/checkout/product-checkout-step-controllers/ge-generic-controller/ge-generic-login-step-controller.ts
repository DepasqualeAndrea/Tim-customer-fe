...
import { GeGenericStepControllerHelper } from './ge-bike-step-controller-helper';

@Injectable({
    providedIn: 'root'
})
export class GeGenericLoginStepController implements ProductLoginRegisterStepController {
    ...
    private addLoggedUserToInsuranceSubject(requestOrder: RequestOrder, responseOrder: ResponseOrder): RequestOrder {
        ...
        if (insuranceSubjects.length !== currentQuantity) {
         ...
         return GeGenericStepControllerHelper.addUserToInsuranceSubject(requestOrder, responseOrder, this.authService.loggedUser);
    }
}
