...
import { catchError,  map, take, takeUntil, tap, toArray } from 'rxjs/operators';
...
export class CheckoutStepAddressComponent implements OnInit {
  ...
  constructor(
    ...
    public netCyberBusinessCheckoutService: NetCyberBusinessCheckoutService,
    private kenticoTranslateService: KenticoTranslateService,
    private checkoutService: NetCyberBusinessCheckoutService,
  ...
  private postalCodeValidator(): AsyncValidatorFn {
    return (control: AbstractControl) => {
      ...
      return this.getPostalCodes(city, state).pipe(
        map(zips =>
          zips.includes(control.value) ? null : { invalidZipForCity: true }
        ),
        catchError(() => of(null)),
        take(1)
      );
