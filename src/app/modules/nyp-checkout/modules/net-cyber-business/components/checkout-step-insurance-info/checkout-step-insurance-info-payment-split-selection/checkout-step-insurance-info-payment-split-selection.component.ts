...
export class CheckoutStepInsuranceInfoPaymentSplitSelectionComponent implements OnInit {
  ...
  private getKenticoRate(code: string): string {
    switch (code) {
      case 'M':
        return this.kenticoContent?.monthly?.value || '';
      case 'T':
        return this.kenticoContent?.split_quarter?.value || '';
      case 'S':
        return this.kenticoContent?.split_semester?.value || '';
      case 'Y':
        return this.kenticoContent?.yearly?.value || '';
      default:
        return '';
    }
  }
     ...
     this.apiService.quote(quoteParams).subscribe({
      ...
      this.content = {
        ...
        insuranceDurationButtons: res.splitPayments.map((o: any) => ({
          ...
          title: this.getKenticoRate(o.description),
      ...
      this.options = res.splitPayments.map((opt: any) => ({
        ...
        price: opt.price,
        discountedPrice: res.splitPayments.find(p => p.description === 'Y')?.price || null,
        default: opt.default,
        label: opt.label
      }));

      const savedFrequency = order?.paymentFrequency || order?.orderItem?.[0]?.instance?.paymentFrequency;
      const selected = this.options.find(o => o.description === savedFrequency) || this.options.find(o => (o.description === 'M' || o.default));
  ...
  private updateOrderAndQuote(): Observable<any> {
    ...
    const updateData: any = {
      paymentFrequency: this.selectedOption.description,
      number_employees_exceeded: true 
    };

    let updatedOrder: any;

    return this.apiService.putOrder(updateData).pipe(
      tap(res => {
        updatedOrder = res;
