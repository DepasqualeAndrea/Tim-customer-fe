...
import { CheckoutStates, CyberCustomerInsuredItems, IOrderResponse, RecursivePartial } from 'app/modules/nyp-checkout/models/api.model';
...
export class CheckoutStepConsensusesComponent implements OnInit {
      ...
      Object.entries(value)
        .forEach(([k, v]) => {
          if (this.latestForm[k] != v && !!v) {
            ...
            switch (k) {
              case 'uno': {
                source = this.kentico.transform('checkout_cyber_consumer.documents_acceptance_checkbox_1');
              }; break;
              case 'due': {
                source = this.kentico.transform('checkout_cyber_consumer.documents_acceptance_checkbox_2');
              }; break;
            }
  ...
  private updateOrder(): Observable<(RecursivePartial<IOrderResponse<CyberCustomerInsuredItems>> | void)[]> {
    const insuredItems = {
     seller_code: this.documentsAcceptancesForm.get('codeLabel').value
    }
