...
import { TimCyberConsumerCheckoutService } from '../../../net-cyber-consumer/services/checkout.service';
...
export class CheckoutStepLoginComponent implements OnInit {
  ...
  constructor(
    ...
    private checkoutService: TimCyberConsumerCheckoutService,
  ...
  getKenticoContent() {
    this.kenticoTranslateService.getItem<any>('access_customers').pipe(
      take(1),
      tap(() => console.log(TIM_CYBER_CONSUMER_PRODUCT_NAME, 'processing old-fashioned login translation')),
      map(kenticoItem => ({
      ...
      tap(() => console.log(TIM_CYBER_CONSUMER_PRODUCT_NAME, 'processed old-fashioned translation')),
