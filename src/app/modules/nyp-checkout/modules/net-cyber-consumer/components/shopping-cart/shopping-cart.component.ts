...
import { IPacketNWarranties, TimCyberConsumerCheckoutService } from '../../../net-cyber-consumer/services/checkout.service';
  ...
  ngOnInit(): void {
    this.nypDataService.CurrentProduct$.pipe(take(1)).subscribe(product => {
      ...
      product
        ...
        ?.forEach(p => {
          switch (p?.sku) {
            case 'net-cyber-consumer-silver': {
              this.packetAndWarranties.set('Silver', { packet: 'checkout_cyber_consumer.insurance_info_silver_title', warranties: p.warranties?.map(w => w.translationCode)});
            }; break;
            case 'net-cyber-consumer-gold': {
              this.packetAndWarranties.set('Gold', { packet: 'checkout_cyber_consumer.insurance_info_gold_title', warranties: p.warranties?.map(w => w.translationCode) });
            }; break;
            case 'net-cyber-consumer-platinum': {
              this.packetAndWarranties.set('Platinum', { packet: 'checkout_cyber_consumer.insurance_info_platinum_title', warranties: p.warranties?.map(w => w.translationCode) });
            }; break;
          }
        });
      });
