...
import { UserType } from 'app/core/models/user-type.model';
...
export class NypGupService {
  ...
  */
  public getUserGupWalletList(gupPaymentType: string, product: CheckoutStepPaymentProduct): Observable<GupPaymentWalletList> {
    return this.nypDataService.CurrentProduct$.pipe(
      map(cp => {
        const paymentMethod = cp
          ?.productsPaymentMethods
          ?.find(paymentMethod => paymentMethod?.paymentMethod?.provider == gupPaymentType)
          ?.paymentMethod;

        this.gupLegacyPaymentMethod = paymentMethod ? {
  ...
  */
  public pay(payload: GupRequest): Observable<any> {
    ...
    payload.amount = this.nypDataService.Order$.value.insurancePremium;
