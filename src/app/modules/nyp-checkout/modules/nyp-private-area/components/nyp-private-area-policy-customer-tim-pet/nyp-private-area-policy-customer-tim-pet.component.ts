...
export class NypPrivateAreaPolicyCustomerTimPetComponent implements OnInit {
  ...
  @Input() public order: IOrderResponse<any>;
  ...
  getPolicyStatus(policy: NypPolicy): string {
    const policyStatusObject =
      NypPrivateAreaPolicyCustomerTimPetComponent.PolicyStatus[policy.status];
  ...
  private setPetInfos(policy): void {
    ...
    const insuredPet = policy.insured_entities.pets[0];
  ...
  openModalPaymentChange() {
    this.modalService.open(NypConfirmChangePaymentMethodComponent, {
      size: "sm",
