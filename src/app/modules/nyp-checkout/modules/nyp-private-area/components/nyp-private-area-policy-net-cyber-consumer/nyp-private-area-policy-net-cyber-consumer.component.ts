...
export class NypPrivateAreaPolicyNetCyberConsumerComponent implements OnInit {
  ...
  ngOnInit() {
       this.kenticoTranslateService.getItem<any>('nyp_product_data').pipe().subscribe(item => {
  ...
  getPolicyStatus(policy: NypPolicy): string {
    const policyStatusObject = NypPrivateAreaPolicyNetCyberConsumerComponent.PolicyStatus[policy.status];
