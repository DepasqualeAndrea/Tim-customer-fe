...
export class CheckoutStepLoginComponent implements OnInit {
  ...
  constructor(
    ...
    private checkoutService: TimBillProtectionCheckoutService,
  ...
  getHeaderLabel(): string {
    return this.activatedRoute.snapshot.fragment === RouteHash.REGISTER ? 'registration_header' : 'login_header';
  }

  getKenticoContent() {
    this.kenticoTranslateService.getItem<any>('access_customers').pipe(
      take(1),
      tap(() => console.log(TIM_BILL_PROTECTION_2_PRODUCT_NAME, 'processing old-fashioned login translation')),
      map(kenticoItem => ({
      ...
      tap(() => console.log(TIM_BILL_PROTECTION_2_PRODUCT_NAME, 'processed old-fashioned translation')),
