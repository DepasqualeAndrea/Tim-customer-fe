...
export class CheckoutStepConsensusesComponent implements OnInit {
              ...
              case "uno":
                {
                  source = this.kentico.transform(
                    "tim_bill_protection.checkbox_documents_acceptance_1_bp"
                  );
                }
                break;
              case "tre":
                {
                  source = this.kentico.transform(
                    "tim_bill_protection.checkbox_documents_acceptance_3_bp"
                  );
                }
                break;
            }
  ...
  public handleNextStep() {
    ...
    this.kenticoTranslateService.getItem<any>('tim_bill_protection').pipe(take(1)).subscribe(item => {
      const stepName = item?.payment_label_bp?.value;
