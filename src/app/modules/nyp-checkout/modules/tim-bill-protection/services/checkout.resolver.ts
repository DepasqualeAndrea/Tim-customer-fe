...
import { TIM_BILL_PROTECTION_KENTICO_NAME, TIM_BILL_PROTECTION_KENTICO_SLUG, TIM_BILL_PROTECTION_PRODUCT_NAME } from 'app/modules/nyp-checkout/nyp-checkout.module';
...
export class TimBillProtectionCheckoutResolver implements Resolve<boolean> {
  ...
  resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<boolean> {
    console.log(TIM_BILL_PROTECTION_PRODUCT_NAME, 'RESOLVER');
    return this.apiService.getProduct().pipe(
      filter(product => !!product),
      take(1),
      map(product => product.packets.find(p => !!p.preselected)),
      tap(packet => {
        TimBillProtectionCheckoutService.InsuranceInfoState$.next('insured-documents');
      ...
      mergeMap(packet => zip(
        this.nypDataService.downloadKenticoContent(TIM_BILL_PROTECTION_KENTICO_NAME, TIM_BILL_PROTECTION_KENTICO_SLUG),
        this.nypDataService.downloadKenticoContent('tim_products_steppers', 'tim_products_steppers'),
