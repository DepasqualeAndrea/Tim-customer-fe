...
export class CheckoutStepAddressComponent implements OnInit {
  ...
  summaryStates: CheckoutStates[] = ['survey', 'consensuses', 'insurance-info', 'user-control'];
  ...
  constructor(
    ...
    public timBillProtectorcheckoutService: TimBillProtectorCheckoutService,
  ) { }
  ...
  ngOnInit(): void {
    ...
    this.nypDataService.CurrentState$.pipe(
      ...
      if(!this.timBillProtectorcheckoutService?.ChosenPackets$?.value?.price){
        const order = this.nypDataService.Order$.value;
        if(order){
          const orderItem = order?.orderItem[0];
          const orderPacket = order?.packet?.data;
          const warranties = order?.packet?.data?.warranties?.map(warranty => ({
            ...warranty,
            startDate: orderItem?.start_date,
            endDate: orderItem?.expiration_date
          }));
          this.timBillProtectorcheckoutService.ChosenPackets$.next({
            packet: orderPacket,
            warranties,
            price: orderPacket?.packetPremium
          });
        }
      }
