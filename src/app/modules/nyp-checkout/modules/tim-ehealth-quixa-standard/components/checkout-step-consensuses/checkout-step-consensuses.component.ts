...
export class CheckoutStepConsensusesComponent implements OnInit {
      ...
      Object.entries(value)
        .forEach(([k, v]) => {
          if (this.latestForm[k] != v && !!v) {
            ...
            switch (k) {
              case 'uno': {
                source = this.kentico.transform('tim_ehealth_quixa_standard.checkbox_documents_acceptance_1_eqs');
              }; break;
              case 'due': {
                source = this.kentico.transform('tim_ehealth_quixa_standard.checkbox_documents_acceptance_2_eqs');
              }; break;
            }
  ...
  public handleNextStep() {
    ...
    this.kenticoTranslateService.getItem<any>('checkout_ehealth_quixa').pipe(take(1)).subscribe(item => {
      let digitalData: digitalData = window["digitalData"];
      digitalData.page.pageInfo.pageName = 'Pagamento';
      this.adobeAnalyticsDataLayerService.pushAdobeCustomTags(digitalData);
    });
