...
export class CheckoutStepLoginComponent implements OnInit {
  ...
  constructor(
    ...
    private checkoutService: TimEhealthQuixaStandardCheckoutService,
  ...
  getKenticoContent() {
    this.kenticoTranslateService.getItem<any>('access_customers').pipe(
      take(1),
      tap(() => console.log(TIM_EHEALTH_QUIXA_STANDARD_PRODUCT_NAME, 'processing old-fashioned login translation')),
      map(kenticoItem => ({
      ...
      tap(() => console.log(TIM_EHEALTH_QUIXA_STANDARD_PRODUCT_NAME, 'processed old-fashioned translation')),
