...
import { BillProtectionInsuredItems, IOrderManagerRequest, IOrderRequest, IOrderResponse, IProduct, IQandA, RecursivePartial, UpdateOrderRequest } from 'app/modules/nyp-checkout/models/api.model';
...
export class TimEhealthQuixaStandardApiService {
  private readonly PATHS = {
    GET_PRODUCT: '/api/latest/products',
    POST_ORDER: '/api/latest/order',
    POST_ORDER_LEGACY_YIN: '/api/v1/orders',
  ...
  public GET_EMISSIONBODY(orderCode: string, productId?: number): IOrderManagerRequest {
    return {
      "addons": null,
      "channel": 'legacy',
      "order":
  ...
  public getProduct(): Observable<RecursivePartial<IProduct>> {
    ...
    return this.nypDataService.Products$
      .pipe(
        map(response => response.find(product => product.code == TIM_EHEALTH_QUIXA_STANDARD_PRODUCT_NAME)),
  ...
  public getOrder(idOrder: string): Observable<RecursivePartial<IOrderResponse<BillProtectionInsuredItems>>> {
    return this.httpClient.get<{ data: RecursivePartial<IOrderResponse<BillProtectionInsuredItems>> }>(this.PATHS.GET_ORDER + idOrder)
      .pipe(
        ...
        tap(() => TimEhealthQuixaStandardCheckoutService.loadFieldToRecover()),
      );
  }
  ...
  public getOrderById(idOrder: string): Observable<RecursivePartial<IOrderResponse<BillProtectionInsuredItems>>> {
    return this.httpClient.get<{ data: RecursivePartial<IOrderResponse<BillProtectionInsuredItems>> }>(this.PATHS.GET_ORDER_ID + idOrder)
      .pipe(
        map(response => response.data),
        tap(response => this.nypDataService.Order$.next(response)),
        tap(response => TimEhealthQuixaStandardCheckoutService.fieldsToRecover = response.fieldToRecover as FieldsToRecover),
        tap(() => TimEhealthQuixaStandardCheckoutService.loadFieldToRecover())
      );
  }
  ...
  public putOrder(data: RecursivePartial<UpdateOrderRequest<BillProtectionInsuredItems>> = {}): Observable<RecursivePartial<IOrderResponse<BillProtectionInsuredItems>>> {
    const fields2recover = TimEhealthQuixaStandardCheckoutService.persistFieldToRecover();
      ...
      data: {
        ...
        chosenWarranties: {
          data: {
            ...
            warranties: data?.packet?.warranties,
  ...
  public createOrderLegacyDoNotUseJustForYIN(): Observable<{ code: string }> {
    return this.httpClient.post<any>(this.PATHS.POST_ORDER_LEGACY_YIN, {
      "order":
      {
        "line_items_attributes":
        {
        ...
        "order_attributes":
          { "product": "ehealth-quixa-standard" }
      },
      "version": "v2"
    })
      .pipe(map(response => ({
