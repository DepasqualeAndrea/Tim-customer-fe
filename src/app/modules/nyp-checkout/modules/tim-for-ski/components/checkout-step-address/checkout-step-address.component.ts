...
export class CheckoutStepAddressComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.nypDataService.CurrentState$.pipe(
      ...
      this.form.setValidators((group: FormGroup): ValidationErrors => {
        ...
        // Controlla se l'assicurato è il contraente
        const order = this.nypDataService.Order$.value;
        const insuredIsContractor = (order?.orderItem?.[0]?.insured_item as any)?.insured_is_contractor;

        // Se l'assicurato NON è il contraente, salta la validazione dell'età
        ...
        if (!product || !product.holder_minimum_age || !product.holder_maximum_age) {
          return null;
        }

        if (moment().subtract(product.holder_minimum_age, 'years').isBefore(birthDate)) {
          return { notCompliant: 'tim_for_ski.age_too_young_error' };
        }

        // (es. 70 anni - 1 giorno = 69 anni e 364 giorni)
        if (moment().subtract(product.holder_maximum_age, 'years').add(1, 'days').isAfter(birthDate)) {
          return { notCompliant: 'tim_for_ski.age_too_old_error' };
        }
  ...
  fromViewToModel(): any {
    ...
    return {
      birth_country: this.form.controls['birthCountry'].value?.name,
      birth_country_id: this.form.controls['birthCountry'].value?.id,
      birth_state: this.form.controls['birthState'].value?.name,
      birth_state_id: this.form.controls['birthState'].value?.id,
      birth_state_abbr: this.form.controls['birthState'].value?.iso,
      birth_city: this.form.controls['birthCity'].value?.name,
      birth_city_id: this.form.controls['birthCity'].value?.id,
      street: this.form.controls['residentialAddress'].value,
      zip_code: this.form.controls['postCode'].value,
      city: this.form.controls['residentialCity'].value?.name,
      city_id: this.form.controls['residentialCity'].value?.id,
      country: this.form.controls['residentialCountry'].value?.name,
      country_id: this.form.controls['residentialCountry'].value?.id,
      state: this.form.controls['residentialState'].value?.name,
      state_id: this.form.controls['residentialState'].value?.id,
      state_abbr: this.form.controls['residentialState'].value?.abbr,
      id: this.authService.loggedUser.id,
      date_of_birth: this.form.controls['birthDate'].value,
      primary_mail: this.form.controls['email'].value,
      primary_phone: this.form.controls['phoneNumber'].value,
      name: this.form.controls['firstName'].value,
      surname: this.form.controls['lastName'].value,
      tax_code: this.form.controls['fiscalCode'].value,
      gender: genderValue
    };
  }
}
