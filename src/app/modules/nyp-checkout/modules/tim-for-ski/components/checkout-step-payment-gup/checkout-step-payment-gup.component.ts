      ...
      Object.entries(value)
        .forEach(([k, v]) => {
          if (this.latestForm[k] != v && !!v) {
            ...
            switch (k) {
              case 'uno': {
                source = this.kentico.transform('tim_for_ski.documents_acceptance_checkbox_1_sci');
              }; break;
              case 'due': {
                source = this.kentico.transform('tim_for_ski.documents_acceptance_checkbox_2_sci');
              }; break;
              // case 'tre': {
  ...
  private gupPaymentAction(selectedPaymentMethod: GupPaymentMethod, selectedPitype: Pitype): void {
    ...
    concat(
      ...
      this.apiService.putOrder({
        ...
        price: this.nypDataService.Order$.value.packet?.data?.packetPremium,
        insuredItems: this.dataService.insuredItem,
