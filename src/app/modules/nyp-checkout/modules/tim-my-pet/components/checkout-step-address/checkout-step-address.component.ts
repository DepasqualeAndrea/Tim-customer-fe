...
import { smartWarranties, smartPlusWarranties, deluxeWarranties } from '../checkout-step-insurance-info/checkout-step-insurance-info.component';
...
export class CheckoutStepAddressComponent implements OnInit {
  ...
  constructor(
    ...
    public timMyPetCheckoutService: TimMyPetCheckoutService,
  ...
  ngOnInit(): void {
    ...
    this.nypDataService.CurrentState$.pipe(
      ...
      if (!this.timMyPetCheckoutService?.ChosenPackets$?.value?.price) {
        const order = this.nypDataService.Order$.value;
        if (order) {
          const orderPacket = order?.packet?.data;
          const sku = order.packet.data?.sku;
          const warranties = this.getSelectedPacketInfo(sku as PacketsName).warranties;
          this.timMyPetCheckoutService.ChosenPackets$.next({
            packet: orderPacket,
