      ...
      Object.entries(value)
        .forEach(([k, v]) => {
          if (this.latestForm[k] != v && !!v) {
            ...
            switch (k) {
              case 'uno': {
                source = this.kentico.transform('customers_tim_pet.documents_acceptance_checkbox_1_mp');
              }; break;
              case 'due': {
                source = this.kentico.transform('customers_tim_pet.documents_acceptance_checkbox_2_mp');
              }; break;
              // case 'tre': {
