...
import { CheckoutStates, IOrderResponse, NatCatnsuredItems, RecursivePartial } from 'app/modules/nyp-checkout/models/api.model';
...
export class CheckoutStepConsensusesComponent implements OnInit {
  ...
  handleSellerCode(): void {
    this.updateOrder().subscribe({
      next: (response: (RecursivePartial<IOrderResponse<NatCatnsuredItems>> | void)[]) => {
        this.isSuccess = true;
  ...
  public downloadDocument(field: 'informationSet' | 'privacy'): void {
    const node = field === 'informationSet' ? this.content.step_payment.information_set : this.content.step_payment.privacy_consens;
