...
export class CheckoutStepConsensusesComponent implements OnInit {
  ...
  public kenticoTranslateSetInformativa: string = this.kentico.transform('tim_protezione_casa.checkbox_documents_acceptance_1');
  ...
  ngOnInit(): void {
    ...
    this.documentsAcceptancesForm.valueChanges.subscribe((value: { uno: boolean, /* due: boolean, */ tre: boolean }) => {
      ...
      Object.entries(value)
        .forEach(([k, v]) => {
          if (this.latestForm[k] != v && !!v) {
            ...
            switch (k) {
              case 'uno': {
                source = this.kenticoTranslateSetInformativa;
              }; break;
              /* case 'due': {
                source = this.kentico.transform('tim_protezione_casa.checkbox_documents_acceptance_2');
              }; break; */
              case 'tre': {
                source = this.kentico.transform('tim_protezione_casa.checkbox_documents_acceptance_3');
              }; break;
            }
      ...
      const condition = `${value?.uno ? 'Accetto uno' : ''} ${value?.tre ? 'Accetto tre' : ''}`;
    ...
    this.nypDataService.CurrentState$.pipe(
      ...
      this.checkoutService.ChosenPackets$.pipe(filter(cp => !!cp?.packetCombo)).subscribe(cp =>
        this.kenticoTranslateSetInformativa = this.kenticoTranslateSetInformativa
          .replace('__________blank__________', this.nypDataService.CurrentProduct$.value?.packets?.find(p => p.sku == cp.packetCombo)?.configuration?.packetDoc)
      );
    });
  }
  ...
  public handleApplyPromoCode(promoCode: CheckoutStepPaymentPromoCode) {
    ...
    this.nypCheckoutService.applyCouponCode(this.nypDataService.Order$.value.id, promoCode.value, this.nypDataService.Order$.value.orderCode, this.nypDataService.Order$.value.orderItem?.[0]?.id)
      .pipe(
        ...
        mergeMap(() => this.apiService.getOrderByCode(this.nypDataService.Order$.value.orderCode))
      )
      .subscribe(
        response => {
