...
import { TimProtezioneViaggiAnnualeCheckoutService } from '../../services/checkout.service';
...
export class CheckoutOrchestratorComponent implements OnInit {
  ...
  constructor(public checkoutService: TimProtezioneViaggiAnnualeCheckoutService, public nypDataService: NypDataService) { }

  ngOnInit(): void {
    const firstState = this.nypDataService.Yin ? 'insurance-info' : 'login-register'
    this.nypDataService.CurrentState$.next(this.nypDataService.StateAfterRedirect ?? firstState);
