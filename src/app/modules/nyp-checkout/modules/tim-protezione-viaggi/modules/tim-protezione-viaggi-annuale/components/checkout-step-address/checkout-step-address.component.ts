...
export class CheckoutStepAddressComponent implements OnInit {
  ...
  summaryStates: CheckoutStates[] = ['insurance-info', 'survey', 'consensuses'];
  ...
  ngOnInit(): void {
    this.endUser = {
      ...
      birth_date: this.authService.loggedUser?.birth_date,
  ...
  updateUser() {
    ...
    concat(
      ...
      this.kenticoTranslateService.getItem<any>('tim_protezione_viaggi').pipe(take(1)).subscribe(item => {
        const stepName = item?.insurance_info_title_viaggi?.value;
