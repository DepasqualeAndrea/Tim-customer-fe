...
@Component({
  ...
  styleUrls: ['./checkout-step-consensuses.component.scss',
...
export class CheckoutStepConsensusesComponent implements OnInit {
  ...
  public code = this.nypDataService.Order$.value.packet.data.product.code;
      ...
      Object.entries(value)
        .forEach(([k, v]) => {
          if (this.latestForm[k] != v && !!v) {
            ...
            switch (k) {
              case 'uno': {
                if (this.code === 'tim-protezione-viaggi-europe-y') {
                  source = this.kentico.transform('tim_protezione_viaggi.documents_set_informativo_annuale_europa_viaggi');
                } else {
                  source = this.kentico.transform('tim_protezione_viaggi.documents_set_informativo_annuale_mondo_viaggi');
                }
              }; break;
              case 'due': {
                source = this.kentico.transform('tim_protezione_viaggi.documents_privacy_annuale_viaggi');
              }; break;
            }
  ...
  public handleApplySellerCode(sellerCode: CheckoutStepPaymentPromoCode) {
    let digitalData: digitalData = window['digitalData'];
    digitalData.cart.form.button_name = this.nypDataService.CurrentState$.value + '_' + "Applica codice promo" + sellerCode;
    this.adobeAnalyticsDataLayerService.adobeTrackClick();
  }

  public handlePrevStep() {
    let digitalData: digitalData = window['digitalData'];
    digitalData.cart.form.button_name = this.nypDataService.CurrentState$.value + '_' + document.getElementById('button-back').textContent.toLowerCase().replace(/\s/g, '');
