...
export class ShoppingCartComponent implements OnInit {
  ...
  public packetName = this.nypDataService.Order$.value.packet.data.name;
  ...
  ngOnInit(): void {
    ...
    this.warranties = this.nypDataService.Order$.value?.orderItem[0].instance?.chosenWarranties.data.warranties.map(warranty => warranty.anagWarranty.name);

    this.nypDataService.Order$.subscribe(order => {
      if (order) {
        this.documentToDownload = order?.packet?.data?.configuration?.packetDoc;
        this.loadWarranties();
      }
    });
  }
  ...
  private loadWarranties() {
    ...
    const packetWarranties = this.nypDataService.Order$.value?.packet?.data?.warranties || [];
    if (packetWarranties && packetWarranties.length > 0) {

      const productSku = this.nypDataService.Order$.value?.packet?.data?.sku || '';
      const isEurope = productSku.includes('europe');

      const insuredItem = this.nypDataService.Order$.value?.orderItem?.[0]?.insured_item as any;
      const extraOption = insuredItem?.extra;

      packetWarranties.forEach(warranty => {
        if (warranty && warranty.anagWarranty) {
          const code = warranty.anagWarranty.internal_code?.toUpperCase();

          if (isEurope) {
            if (['EAM', 'EAA'].includes(code)) {
              this.warranties.push(warranty.anagWarranty.name);
            }

            else if (extraOption && extraOption !== 'none' && code === extraOption.toUpperCase()) {
              this.warranties.push(warranty.anagWarranty.name);
            }
          }

          else {
            if (['EAM', 'EAA', 'EARC'].includes(code)) {
              this.warranties.push(warranty.anagWarranty.name);
            }
            else if (extraOption && extraOption !== 'none' && code === extraOption.toUpperCase()) {
              this.warranties.push(warranty.anagWarranty.name);
