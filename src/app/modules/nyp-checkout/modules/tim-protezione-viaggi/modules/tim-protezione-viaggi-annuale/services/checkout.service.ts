...
import { TIM_PROTEZIONE_VIAGGI_MODULE_NAME, TIM_PROTEZIONE_VIAGGI_EUROPE_Y_ANNUALE_PRODUCT_NAME, TIM_PROTEZIONE_VIAGGI_WORLD_Y_ANNUALE_PRODUCT_NAME, TIM_PROTEZIONE_VIAGGI_ANNUALE_MODULE_NAME, } from 'app/modules/nyp-checkout/nyp-checkout.module';
...
export class TimProtezioneViaggiAnnualeCheckoutService extends FieldToRecover {
  public get InsuranceInfoState$(): BehaviorSubject<InsuranceInfoStates> { return TimProtezioneViaggiAnnualeCheckoutService.InsuranceInfoState$; };
  public static readonly InsuranceInfoState$ = new BehaviorSubject<InsuranceInfoStates>('choicePet');

  public get ChosenPackets$(): BehaviorSubject<IPacketNWarranties | undefined> { return TimProtezioneViaggiAnnualeCheckoutService.ChosenPackets$; };
  public static readonly ChosenPackets$ = new BehaviorSubject<IPacketNWarranties | undefined>(undefined);

  public get ChosenPacketsName(): string[] { return TimProtezioneViaggiAnnualeCheckoutService.ChosenPacketsName; };
  public set ChosenPacketsName(value: string[]) { TimProtezioneViaggiAnnualeCheckoutService.ChosenPacketsName = value; };
  public static ChosenPacketsName: string[] = [];

  public get SelectedPackets(): IPacketChoose { return TimProtezioneViaggiAnnualeCheckoutService.SelectedPackets; };
  public set SelectedPackets(value: IPacketChoose) { TimProtezioneViaggiAnnualeCheckoutService.SelectedPackets = value; };
  ...
  constructor(private router: Router, private nypDataService: NypDataService, private dataService: DataService, private adobeAnalyticsDataLayerService: AdobeAnalyticsDatalayerService,
    ...
    this.nypDataService.CurrentState$
      .pipe(filter(e => !!e))
      .subscribe(state => {
        console.log(TIM_PROTEZIONE_VIAGGI_EUROPE_Y_ANNUALE_PRODUCT_NAME,
          TIM_PROTEZIONE_VIAGGI_WORLD_Y_ANNUALE_PRODUCT_NAME, state);
        const fragment = [];
        ...
        this.kenticoTranslateService.getItem<any>('tim_protezione_viaggi').pipe(take(1)).subscribe(item => {
          if(state == 'insurance-info'){
            ...
            const stepName = "tim-protezione-annuale scelta pacchetto";
          ...
          if(state == 'address'){
            const productName = item?.system?.name;
            const stepName = "tim-protezione-annuale dati anagrafici";
          ...
          if(state == 'user-control'){
            const stepName = "tim-protezione-annuale user control";
          ...
          if(state == 'survey'){
            const productName = item?.system?.name;
            const stepName = "tim-protezione-annuale questionario di coerenza";
            let digitalData: digitalData = window["digitalData"];
            digitalData.page.category.primaryCategory = productName;
            digitalData.page.pageInfo.pageName = stepName;
            this.adobeAnalyticsDataLayerService.pushAdobeCustomTags(digitalData);
          ...
          if(state == 'consensuses'){
            const productName = item?.system?.name;
            const stepName = "tim-protezione-annuale consensi";
            let digitalData: digitalData = window["digitalData"];
            digitalData.page.category.primaryCategory = productName;
            digitalData.page.pageInfo.pageName = stepName;
            this.adobeAnalyticsDataLayerService.pushAdobeCustomTags(digitalData);
          ...
          if(state == 'payment'){
            const stepName = "tim-protezione-annuale pagamento";
            let digitalData = window["digitalData"];
            digitalData.page.pageInfo.pageName = stepName;
            this.adobeAnalyticsDataLayerService.pushAdobeCustomTags(digitalData);
          }
        ...
        if (state == 'survey') {
          ...
          console.log(TIM_PROTEZIONE_VIAGGI_EUROPE_Y_ANNUALE_PRODUCT_NAME,
            TIM_PROTEZIONE_VIAGGI_WORLD_Y_ANNUALE_PRODUCT_NAME, 'YIN PAYLOAD', JSON.stringify(dataOrder));
          window.parent.postMessage(dataOrder, '*');
          const stepName = "tim-protezione-annuale loginPage";
          let digitalData = window["digitalData"];
          digitalData.page.pageInfo.pageName = stepName;
          this.adobeAnalyticsDataLayerService.pushAdobeCustomTags(digitalData);
        }
        if (state == 'thank-you') {
          let digitalData: any = window['digitalData'];
          digitalData.page.pageInfo.pageName = `tim-protezione-annuale - thank-you`;
          this.adobeAnalyticsDataLayerService.pushAdobeCustomTags(digitalData);
        }
        // Prevent navigation when the current location is outside the current module
        if (window.location.pathname.includes(`/nyp-checkout/${TIM_PROTEZIONE_VIAGGI_ANNUALE_MODULE_NAME}`))
          this.router.navigate([`/nyp-checkout/${TIM_PROTEZIONE_VIAGGI_ANNUALE_MODULE_NAME}/${state}`], ...fragment);
      });
  }
}
