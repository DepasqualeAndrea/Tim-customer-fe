      ...
      Object.entries(value)
        .forEach(([k, v]) => {
          if (this.latestForm[k] != v && !!v) {
            ...
            switch (k) {
              case 'uno': {
                if (this.code === 'tim-protezione-viaggi-europe') {
                  source = this.kentico.transform('tim_protezione_viaggi.documents_set_informativo_breve_europa_viaggi');
                } else {
                  source = this.kentico.transform('tim_protezione_viaggi.documents_set_informativo_breve_mondo_viaggi');
                }

              }; break;
              case 'due': {
                if (this.code === 'tim-protezione-viaggi-europe') {
                  source = this.kentico.transform('tim_protezione_viaggi.documents_privacy_breve_europa_viaggi');
                } else {
                  source = this.kentico.transform('tim_protezione_viaggi.documents_privacy_breve_mondo_viaggi');
                }
              }; break;
            }
