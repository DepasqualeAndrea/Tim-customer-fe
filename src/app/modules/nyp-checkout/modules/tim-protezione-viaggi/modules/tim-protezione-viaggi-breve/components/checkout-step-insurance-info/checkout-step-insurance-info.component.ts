...
import { CheckoutStates, IOrderResponse, RecursivePartial, ViaggiBreveInsuredItems } from 'app/modules/nyp-checkout/models/api.model';
...
export class CheckoutStepInsuranceInfoComponent implements OnInit {
  ...
  handleNextStep() {
    ...
    const endDate = orderItem.expiration_date;
    const startDate = orderItem.insured_item.start_date;
    const insuredItems = orderItem.insured_item as ViaggiBreveInsuredItems;
    const destinationCode = insuredItems?.destinationCode;

    const insuredItem: ViaggiBreveInsuredItems = {
      insurance_holders: this.subjects.controls.map((form: UntypedFormGroup) => {
        return {
          surname: form.controls.insuredSurname.value,
          name: form.controls.insuredName.value,
          birth_date: this.formatDateToYMD(new Date(form.controls.insuredDate.value)),
        };
      }),
      destination: this.nypDataService.Order$?.value?.orderItem[0]?.insured_item?.destination,
      destinationCode: destinationCode,
      overSeventy: this.nypDataService.Order$.value.orderItem[0].insured_item.overSeventy,
      underSeventy: this.nypDataService.Order$.value.orderItem[0].insured_item.underSeventy,
      ...
      expiration_date: this.formatDateToISOString(this.nypDataService.Order$.value?.orderItem[0]?.expiration_date),
  ...
  public getValidate(): Observable<RecursivePartial<IOrderResponse<ViaggiBreveInsuredItems>>> {
    try {
      ...
      if (!orderId) {
        ...
        return of({} as RecursivePartial<IOrderResponse<ViaggiBreveInsuredItems>>);
      }
  ...
  private updateOrder(insuredItems?: ViaggiBreveInsuredItems): Observable<(RecursivePartial<IOrderResponse<ViaggiBreveInsuredItems>> | void)[]> {
    ...
    const startDate = new Date(orderItem.insured_item.start_date);
    ...
    return concat(
      this.apiService.putOrder({
        ...
        chosenWarranties: this.nypDataService.Order$.value.packet?.data?.warranties
          ?.map(warranty => Object.assign(warranty, {
            startDate: this.nypDataService.Order$.value?.orderItem[0]?.start_date,
            endDate: this.nypDataService.Order$.value?.orderItem[0]?.expiration_date
          })),
