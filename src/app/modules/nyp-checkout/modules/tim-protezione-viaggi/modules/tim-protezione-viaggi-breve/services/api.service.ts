...
import { ViaggiBreveInsuredItems, IOrderManagerRequest, IOrderRequest, IOrderResponse, IProduct, IQandA, RecursivePartial, UpdateOrderRequest, IQuote } from 'app/modules/nyp-checkout/models/api.model';
import { TIM_PROTEZIONE_VIAGGI_EUROPE_BREVE_PRODUCT_NAME, TIM_PROTEZIONE_VIAGGI_WORLD_BREVE_PRODUCT_NAME } from 'app/modules/nyp-checkout/nyp-checkout.module';
...
export class TimProtezioneViaggiBreveApiService {
  ...
  public getProduct(): Observable<RecursivePartial<IProduct>> {
    ...
    return this.nypDataService.Products$
      .pipe(
        map(response => response.find(product => product.code == TIM_PROTEZIONE_VIAGGI_EUROPE_BREVE_PRODUCT_NAME
          || product.code == TIM_PROTEZIONE_VIAGGI_WORLD_BREVE_PRODUCT_NAME)), filter(viaggi => !!viaggi?.code),
        take(1),
        map(viaggi => {
          viaggi
            .packets
  ...
  public getOrder(idOrder: string): Observable<RecursivePartial<IOrderResponse<ViaggiBreveInsuredItems>>> {
    return this.httpClient.get<{ data: RecursivePartial<IOrderResponse<ViaggiBreveInsuredItems>> }>(this.PATHS.GET_ORDER + idOrder)
      .pipe(
        ...
        tap(() => TimProtezioneViaggiBreveCheckoutService.loadFieldToRecover()),
        tap(() => {
  ...
  public getOrderById(idOrder: string): Observable<RecursivePartial<IOrderResponse<ViaggiBreveInsuredItems>>> {
    return this.httpClient.get<{ data: RecursivePartial<IOrderResponse<ViaggiBreveInsuredItems>> }>(this.PATHS.GET_ORDER_ID + idOrder)
      .pipe(
        map(response => response.data),
        tap(response => this.nypDataService.Order$.next(response)),
        tap(response => TimProtezioneViaggiBreveCheckoutService.fieldsToRecover = response.fieldToRecover as FieldsToRecover),
        tap(() => TimProtezioneViaggiBreveCheckoutService.loadFieldToRecover())
      );
  }
  ...
  public putOrder(data: RecursivePartial<UpdateOrderRequest<ViaggiBreveInsuredItems>>): Observable<RecursivePartial<IOrderResponse<ViaggiBreveInsuredItems>>> {
    ...
    data = {
      ...
      insuredItems: data.insuredItems ?? this.nypDataService.Order$.value?.orderItem?.[0]?.insured_item as RecursivePartial<ViaggiBreveInsuredItems>,
      ...
      days: data.days ?? (this.nypDataService.Order$?.value?.orderItem[0]?.insured_item as ViaggiBreveInsuredItems).days,
    ...
    return this.httpClient.put<{ data: RecursivePartial<IOrderResponse<ViaggiBreveInsuredItems>> }>(
      this.PATHS.PUT_ORDER + data?.orderCode,
      {
        data: {
          ...
          orderItem: [{
            ...
            insured_item: {
              ...data.insuredItems,
              seller_code: data.insuredItems?.seller_code ?? (this.nypDataService.Order$.value?.orderItem?.[0]?.insured_item as ViaggiBreveInsuredItems).seller_code
            },
            days: data.days ?? (this.nypDataService.Order$.value.orderItem[0]?.insured_item as ViaggiBreveInsuredItems)?.days,
          }],
      ...
      } as RecursivePartial<IOrderRequest<ViaggiBreveInsuredItems>>)
      .pipe(
        map(response => response.data),
        tap(response => this.nypDataService.Order$.next(response)),
      );
  ...
  public checkExistingPolicy(): Observable<{ data: string }> {
    const orderId = this.nypDataService.OrderCode || localStorage.getItem('id_order');
    if (!orderId) {
  ...
  public getValidate(): Observable<RecursivePartial<IOrderResponse<ViaggiBreveInsuredItems>>> {
    const orderId = this.nypDataService.Order$.value.orderCode;
    if (!orderId) {
      throw new Error('OrderCode non è definito o è vuoto.');
    }
    return this.httpClient.get<{ data: RecursivePartial<IOrderResponse<ViaggiBreveInsuredItems>> }>(`${this.PATHS.GET_VALIDATE.replace('{orderCode}', orderId)}`)
      .pipe(
        map(response => response.data),
      );
  }
