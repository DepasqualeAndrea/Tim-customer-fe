...
export class CheckoutStepAddressComponent implements OnInit {
  ...
  titleStates: CheckoutStates[] = ['login-register', 'user-control'];
  ...
  ngOnInit(): void {
    this.authService.userChangedSource.subscribe((user) => {
      if (user) {
        this.endUser = {
          name: user.firstname,
          surname: user.lastname,
          tax_code: user.address?.taxcode,
        ...
        this.form = this.formBuilder.group(this.fromModelToView(user));
        ...
        this.getCountry(this.dataService.countriesEndpoint).subscribe(
          (countries) => {
            this.countries = countries;

            if (this.form.controls["birthCountry"].value?.id) {
              this.getState(
                this.form.controls["birthCountry"].value.id,
                "birth"
              ).subscribe();
            ...
            if (this.form.controls["residentialCountry"].value?.id) {
              this.getState(
                this.form.controls["residentialCountry"].value.id,
                "residential"
              ).subscribe();
            }
  ...
  getCity(
    id: number,
    destination: "birth" | "residential" | "all"
  ): Observable<City[]> {
    return this.nypUserService.getCities(id).pipe(
      tap((cities) => {
        if (destination == "birth" || destination == "all") {
          ...
          if (!this.form.controls["birthCity"].value)
            this.form.controls["birthCity"].enable();
        }
    ...
    return this.nypUserService.getProvince(id).pipe(
      tap((states) => {
        if (destination == "birth" || destination == "all") {
          ...
          if (
            destination == "birth" &&
            birthCountryValue?.name?.toLowerCase() !== "italia"
          ) {
            ...
            this.getCity(
              this.form.controls["birthState"].value.id,
              "birth"
            ).subscribe();
        ...
        if (destination == "residential" || destination == "all") {
          ...
          if (this.form.controls["residentialState"].value) {
            this.getCity(
              this.form.controls["residentialState"].value.id,
              "residential"
            ).subscribe();
          }
        }
  ...
  checkConditions() {
    const validCountries = ["Italia", "San Marino", "Vaticano"];
    if (
      validCountries.includes(
        this.form.controls["residentialCountry"].value.name
      ) &&
      this.overEighteen()
    )
      this.isValid = true;
  ...
  selectChanged(csc: { id: number }, nextControlName: string) {
    ...
    const destination = nextControlName.toLowerCase().includes("birth")
      ? "birth"
      : "residential";
