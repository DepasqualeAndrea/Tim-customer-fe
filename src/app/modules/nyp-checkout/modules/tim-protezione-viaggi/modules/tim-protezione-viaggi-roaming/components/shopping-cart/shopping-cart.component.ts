...
export class ShoppingCartComponent implements OnInit {
  ...
  public showStates: CheckoutStates[] = ['login-register', 'address', 'insurance-info', 'survey', 'payment', 'consensuses', 'thank-you', 'user-control'];
  ...
  ngOnInit(): void {
    ...
    this.endDate = this.formatDateToDMY(new Date(this.nypDataService.Order$.value?.orderItem[0]?.expiration_date));
    ...
    this.warranties = this.nypDataService.Order$.value.packet.data.warranties.map(warranty => warranty.anagWarranty.name);
