...
import { ViaggiRoamingInsuredItems, IOrderManagerRequest, IOrderRequest, IOrderResponse, IProduct, IQandA, RecursivePartial, UpdateOrderRequest, IQuote, ICountries } from 'app/modules/nyp-checkout/models/api.model';
...
import { TimProtezioneViaggiRoamingCheckoutService, } from './checkout.service';
import { DataService } from '@services';

@Injectable({
  providedIn: 'root'
})
export class TimProtezioneViaggiRoamingApiService {
  ...
  public getProduct(): Observable<RecursivePartial<IProduct>> {
    ...
    return this.nypDataService.Products$
      .pipe(
        map(response => response.find(product => product.code == TIM_PROTEZIONE_VIAGGI_ROAMING_PRODUCT_NAME)),
        ...
        tap(viaggi => this.nypDataService.CurrentProduct$.next(viaggi)),
      );
  }

  public getOrder(idOrder: string): Observable<RecursivePartial<IOrderResponse<ViaggiRoamingInsuredItems>>> {
    return this.httpClient.get<{ data: RecursivePartial<IOrderResponse<ViaggiRoamingInsuredItems>> }>(this.PATHS.GET_ORDER + idOrder)
      .pipe(
        ...
        tap(() => TimProtezioneViaggiRoamingCheckoutService.loadFieldToRecover()),
        tap(() => {
  ...
  public getOrderById(idOrder: string): Observable<RecursivePartial<IOrderResponse<ViaggiRoamingInsuredItems>>> {
    return this.httpClient.get<{ data: RecursivePartial<IOrderResponse<ViaggiRoamingInsuredItems>> }>(this.PATHS.GET_ORDER_ID + idOrder)
      .pipe(
        map(response => response.data),
        tap(response => this.nypDataService.Order$.next(response)),
        tap(response => TimProtezioneViaggiRoamingCheckoutService.fieldsToRecover = response.fieldToRecover as FieldsToRecover),
        tap(() => TimProtezioneViaggiRoamingCheckoutService.loadFieldToRecover())
      );
  }
  ...
  public putOrder(data: RecursivePartial<UpdateOrderRequest<ViaggiRoamingInsuredItems>>): Observable<RecursivePartial<IOrderResponse<ViaggiRoamingInsuredItems>>> {
    ...
    data = {
      ...
      insuredItems: data.insuredItems ?? this.nypDataService.Order$.value?.orderItem?.[0]?.insured_item as RecursivePartial<ViaggiRoamingInsuredItems>,
      ...
      days: data.days ?? (this.nypDataService.Order$?.value?.orderItem[0]?.insured_item as ViaggiRoamingInsuredItems).days,
    ...
    return this.httpClient.put<{ data: RecursivePartial<IOrderResponse<ViaggiRoamingInsuredItems>> }>(
      this.PATHS.PUT_ORDER + data?.orderCode,
      {
        data: {
          ...
          orderItem: [{
            ...
            insured_item: {
              ...data.insuredItems,
              seller_code: data.insuredItems?.seller_code ?? (this.nypDataService.Order$.value?.orderItem?.[0]?.insured_item as ViaggiRoamingInsuredItems).seller_code
            },
            days: data.days ?? (this.nypDataService.Order$?.value?.orderItem[0]?.insured_item as ViaggiRoamingInsuredItems).days,
      ...
      } as RecursivePartial<IOrderRequest<ViaggiRoamingInsuredItems>>)
      .pipe(
        map(response => response.data),
        tap(response => this.nypDataService.Order$.next(response)),
      );
  ...
  public getCountries(): Observable<string[]> {
    ...
    return this.httpClient.get<ICountries>(`${this.PATHS.GET_COUNTRIES.replace('{orderCode}', orderId)}`)
      .pipe(
