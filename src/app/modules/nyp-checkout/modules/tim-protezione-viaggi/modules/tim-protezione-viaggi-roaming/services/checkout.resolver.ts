...
export class TimProtezioneViaggiRoamingCheckoutResolver  {
  ...
  resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<boolean> {
    console.log(TIM_PROTEZIONE_VIAGGI_ROAMING_PRODUCT_NAME, 'RESOLVER');
    ...
    try {
      ...
      console.warn(TIM_PROTEZIONE_VIAGGI_ROAMING_PRODUCT_NAME, 'Error parsing return query params', e);
    }
    ...
    return this.apiService.getProduct().pipe(
      ...
      tap(packet => {
        TimProtezioneViaggiRoamingCheckoutService.InsuranceInfoState$.next('choicePet');
        TimProtezioneViaggiRoamingCheckoutService.ChosenPacketsName = [];
        TimProtezioneViaggiRoamingCheckoutService.ChosenPackets$.next(packet ? {
      ...
      tap((order) => {
        TimProtezioneViaggiRoamingCheckoutService.fieldsToRecover = order.fieldToRecover as FieldsToRecover;
      }),
      map(() => true),
      catchError(err => {
