  ...
  ngOnInit(): void {
    ...
    this.nypDataService.CurrentState$.pipe(
      ...
      this.form.setValidators((group: UntypedFormGroup): ValidationErrors => {
        ...
        const residentialCountry = group.controls['residentialCountry'].value?.name?.toLowerCase();

        // cannot be older then 74 years old plus 364 days
        if (moment().subtract(75, 'years').subtract(364, 'days').isAfter(birthDate))
          return { ageNotCompliant: 'tim_sport.max_75' };
        // Cannot be younger then 18 years old
        // else if (moment().subtract(18, 'years').isBefore(birthDate))
        //   return { ageNotCompliant: 'tim_sport.min_18' };

        // foreign user are not allowed
        if (residentialCountry !== 'italia')
          return { countryNotCompliant: 'tim_sport.foreign_user' };
      });
  ...
  updateUser() {
    ...
    concat(
      ...
      this.kenticoTranslateService.getItem<any>('tim_sport').pipe(take(1)).subscribe(item => {
        ...
        digitalData.cart.form.button_name = this.nypDataService.CurrentState$.value + '_' + "Conferma";
