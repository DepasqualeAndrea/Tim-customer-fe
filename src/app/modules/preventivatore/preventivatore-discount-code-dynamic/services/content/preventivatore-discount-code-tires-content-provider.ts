...
export class PreventivatoreDiscountCountTiresContentProvider implements PreventivatoreDiscountCodeContentProvider {
  ...
  private getContentFromKentico(codes: string[]): Observable<any> {
    return this.kenticoTranslateService.getItem('preventivatore_tires_free').pipe
      (take(1)
  ...
  createContentStructureFromKenticoItem(codes: string[], kenticoItem: any) {
    const structure = {
      ...
      quotator: {
        quotator_name: kenticoItem.quotator_product.value.find(item => item.system.codename === 'covered_tires_name').text.value,
        quotator_description: kenticoItem.quotator_product.value.find(item =>
          item.system.codename === 'covered_tires_short_description').text.value,
        month_free: kenticoItem.quotator_product.value.find(item =>
          item.system.codename === 'three_month_free').text.value,
        ...
        next_price: kenticoItem.quotator_product.value.find(item =>
          item.system.codename === 'next_prize').text.value,
        ...
        next_price_text: kenticoItem.quotator_product.value.find(item =>
          item.system.codename === 'next_prize_text').text.value,
      },
      how_works: {
  ...
  getContent(codes: string[]): Observable<ContentInterface> {
    ...
    return forkJoin([contentFromKentico$, isAuthorized$])
      .pipe(mergeMap(([contentFromKentico, isAuthorized]) => {
        ...
        obj.what_to_know.container_class = this.dataService.tenantInfo.tenant;
        return of(obj);
      }));
  }
