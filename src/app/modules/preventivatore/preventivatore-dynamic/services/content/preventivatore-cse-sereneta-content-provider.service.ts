...
export class PreventivatoreCseSerenetaContentProvider
    ...
    const structure = {
      ...
      quotator: {
        ...
        slash_price: kenticoItem.quotator_product.value.find(item =>
          item.system.codename === 'slash_year').text.value,
        product_collaboration: kenticoItem.quotator_product.value.find(item =>
          item.system.codename === 'sereneta_partner').product_collaboration.value,
        information_package_text: kenticoItem.quotator_product.value.find(item =>
          item.system.codename === 'sereneta_partner').info_package_link.value,
      ...
      what_to_know: {
        ...
        terms_of_service_content: kenticoItem.what_to_know_info_set.value.find(item =>
          item.system.codename === 'what_to_know_sereneta_terms_of_service').text.value
      },
      more_info: {
  ...
  getHowWorksStructure(productCodes: string[], kenticoItem: any) {
    const selected_slide = productCodes.length === 1 ? productCodes[0] : 'sara-sereneta';
    const product_card_1 = productCodes.some(code => code === 'sara-sereneta') ? this.getSerenitySlide(kenticoItem) : null;
    const product_card_2 = productCodes.some(code => code === 'sara-sereneta') ? this.getCardTwoSlide(kenticoItem) : null;
  ...
  private getContentFromKentico(codes: string[]): Observable<any> {
    return this.kenticoTranslateService.getItem('preventivatore_sereneta').pipe
