...
export class PreventivatoreGeSellaHolidayHouseContentProviderService implements PreventivatoreContentProvider {
  ...
  createContentStructureFromKenticoItem(kenticoItem: any) {
    const structure = {
      ...
      how_works: {
        title: kenticoItem.how_works.container_coverages_holiday.title.value,
      ...
      more_info: {
        title: kenticoItem.more_info.value.find(item => item.system.codename === 'more_info_title_holiday_house').text.value,
        subtitle: kenticoItem.more_info.value.find(item => item.system.codename === 'more_info_text_holiday_house').text.value,
      }
    };
    return structure;
  }
  ...
  private getHowWorksStructure(kenticoItem: any) {
    const howWorks = [];
      const howWorkStructure = {
        coverage: kenticoItem.how_works.container_coverages_holiday.coverages.value.map(items => {
          return {
            ...
            benefits: items.benefit.value.map(options => {
              return {
                ...
                id: options.covid_19 ? options.covid_19.value : null,
  ...
  private getGaranzieAggiuntiveStructure(kenticoItem: any) {
    const more_info = {
      ...
      more_info: kenticoItem.what_to_know.more_info_holiday.container.value,
    };
    return more_info;
  }
