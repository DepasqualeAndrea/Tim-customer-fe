...
export class PreventivatoreGeYoloSkiSeasonalContentProviderService implements PreventivatoreContentProvider {
  ...
  private getContentFromKentico(): Observable<any> {
    return this.kenticoTranslateService.getItem('preventivatore_ski_seasonal_genertel').pipe
  ...
  createContentStructureFromKenticoItem(kenticoItem: any) {
    const structure = {
      ...
      quotator: {
        ...
        extra_message: kenticoItem.quotator.value.find(item => item.system.codename === 'extra_message_ski_seasonal').text.value,
      ...
      how_works: {
        ...
        text_section: kenticoItem.how_works.value.find(item => item.system.codename === 'how_works_ski_seasonal_description').text.value,
        product_content: this.getPolicyCoverages(kenticoItem.how_works.value.find(item => item.system.codename === 'how_works_ski_seasonal').container),
      ...
      what_to_know: {
        ...
        text_section: kenticoItem.what_to_know.value.find(item => item.system.codename === 'what_to_know_ski_seasonal').text.value,
        set_informativo_content: kenticoItem.what_to_know.value.find(item => item.system.codename === 'informative_set_plus_premium_seasonal').text.value
      },
      more_info: {
  ...
  getPolicyCoverages(benefits: any) {
    ...
    benefits.value.map(benefit => {
      const policyCoverage = {
        ...
        product_code: benefit.system.codename.endsWith('ski_seasonal_plus') ? 'ge-ski-seasonal-plus' : 'ge-ski-seasonal-premium',
  ...
  private setExtraContentForQuotator(obj: ContentInterface, quotator: any) {
    obj.container_1.products.forEach(product => {
      ...
      product.extra_message = quotator.extra_message.replace(new RegExp('http://\{\{\s*information_package\s*\}\}', 'gi'), product.information_package);
