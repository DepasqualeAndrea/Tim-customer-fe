...
export class PreventivatoreGeYoloTravelContentProviderService implements PreventivatoreContentProvider {
  ...
  createContentStructureFromKenticoItem(kenticoItem: any) {
    const structure = {
      ...
      how_works: {
        ...
        text_section: kenticoItem.how_works.value.find(item => item.system.codename === 'how_works_travel_description').text.value,
        product_content: this.getPolicyCoverages(kenticoItem.how_works.value.find(item => item.system.codename === 'how_works_travel').container),
