...
export class PreventivatoreIntesaSciContentProvider implements PreventivatoreContentProvider {
    private productCodes = ['ergo-mountain'];
  ...
  createContentStructureFromKenticoItem(kenticoItem: any) {
    const structure = {
      ...
      quotator: {
        ...
        copertura_aggiuntiva_covid19_title: kenticoItem.quotator.value.find(item => item.system.codename === 'copertura_aggiuntiva_covid19').title.value,
        copertura_aggiuntiva_covid19_img: kenticoItem.quotator.value.find(item => item.system.codename === 'copertura_aggiuntiva_covid19').image.images[0].url,
        copertura_aggiuntiva_covid19_img_alt: kenticoItem.quotator.value.find(item => item.system.codename === 'copertura_aggiuntiva_covid19').image.images[0].description
      },
      how_works: this.getHowWorksStructure(kenticoItem),
      for_who: this.getWhoIsFor(kenticoItem),
      insurance_coverages: {
        icon: kenticoItem.insurance.insurance_coverages.icon.images[0].url,
        icon_description: kenticoItem.insurance.insurance_coverages.icon.images[0].description,
        title: kenticoItem.insurance.insurance_coverages.title.value,
        coverages: this.getCoveragesStructure(kenticoItem.insurance.insurance_coverages.coverages.value)
      },
      what_to_know: {
  ...
  getHeaderStructure(kenticoItem: any) {
    const header = {
      ...
      description: kenticoItem.header.value[0].description.value
        ...
        : null,
      welcome_message: kenticoItem.header.value[0].welcome.value
        ? kenticoItem.header.value[0].welcome.value
        : null,
      welcome_message_female: kenticoItem.header.value[0].welcome_female.value
  ...
  getCoveragesStructure(kenticoItem: any){
    const coverageContent= kenticoItem.map(content => {
      return{
        coverages: content.text.value
      }
    })
  ...
  getHowWorksStructure(kenticoItem: any) {
    const selected_slide = 'ergo-mountain-gold';
    const howWorks = {
  ...
  getPolicyCoverages(kenticoItem: any) {
    ...
    kenticoItem.how_works.value[0].benefits.value.map(benefit => {
      ...
      const policyCoverage = {
        ...
        product_code: benefit.system.codename.endsWith('sci_gold') ? 'ergo-mountain-gold' : 'ergo-mountain-silver',
  ...
  getContent(codes: string[]): Observable<ContentInterface> {
    ...
    return contentFromKentico$.pipe(switchMap((contentFromKentico) => {
      ...
      obj.container_1 = Object.assign({}, this.setContentToProduct(orderedContentFromInsuranceService, contentFromKentico.header, welcomeMessage));
      obj.container_2 = Object.assign({}, contentFromKentico.how_works);
      obj.container_3 = Object.assign({}, contentFromKentico.for_who);
      obj.container_4 = Object.assign({}, contentFromKentico.insurance_coverages);
      ...
      obj.container_3.container_class = [this.productCodes[0], this.getTenantLayoutClass()];
  ...
  private setContentToProduct(contentFromInsuranceService: any, header: any, welcomeMessage: string) {
    const obj = {
  ...
  private getUserWelcomeMessage(headerFromKentico: any) {
    ...
    const messageFemale = headerFromKentico.welcome_message_female;
  ...
  private setExtraContentForQuotator(obj: ContentInterface, quotator: any) {
    obj.container_1.products.forEach(product => {
      ...
      product.isSelected = product.product_code === 'ergo-mountain-gold';
      product.copertura_aggiuntiva_covid19_title = quotator.copertura_aggiuntiva_covid19_title
      product.copertura_aggiuntiva_covid19_img = quotator.copertura_aggiuntiva_covid19_img
      product.copertura_aggiuntiva_covid19_alt = quotator.copertura_aggiuntiva_covid19_alt
    });
