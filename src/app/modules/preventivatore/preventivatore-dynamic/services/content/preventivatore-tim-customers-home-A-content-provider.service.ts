...
export class PreventivatoreTimCustomersHomeAContentProviderService implements PreventivatoreContentProvider {
  ...
  private getContentFromKentico(): Observable<any> {
    return this.kenticoTranslateService.getItem('preventivatore_home_a').pipe
  ...
  private getDetails(details: any[]) {
    return details.map(detail => {
      return {
        ...
        promo_tags:   this.getPromoTag(detail['promo_tags'].value[0]),
        modal:        this.getModal(detail['modal'].value[0]),
      }
    })
  }
  ...
  private setContentToProduct(contentFromInsuranceService: any, hero: any) {
    const product = contentFromInsuranceService.find(product => product.product_code === 'myHome-A')
    const obj = {
      product: product,
      title: hero.title,
