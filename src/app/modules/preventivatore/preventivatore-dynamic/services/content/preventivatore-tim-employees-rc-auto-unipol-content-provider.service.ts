...
export class PreventivatoreTimEmployeesRCAutoUnipolProviderService implements PreventivatoreContentProvider {
  ...
  private getContentFromKentico(): Observable<any> {
    return this.kenticoTranslateService.getItem('preventivatore_rc_auto_unipol').pipe
  ...
  createContentStructureFromKenticoItem(kenticoItem: any) {
    const structure = {
      hero: {
        ...
        button_arrow_text: kenticoItem.cta.value,
      ...
      activate_offer_agency: {
        ...
        button_list: kenticoItem.find_button_list.value.map(item => {return {title: item.title.value, link: item.text.value}})
      }
    };
    return structure;
  }
