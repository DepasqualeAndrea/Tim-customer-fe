...
export class PreventivatoreYoloAnnullamentoViaggioContentProviderService {

  private productCodes = ['axa-annullament'];
  ...
  private getContentFromKentico(): Observable<any> {
    return this.kenticoTranslateService.getItem('preventivatore_annullamento_viaggio').pipe(map(contentItem => this.createContentStructureFromKenticoItem(contentItem)));
  }
  ...
  createContentStructureFromKenticoItem(kenticoItem: any) {
    ...
    console.log('Kentico annullamento: ', structure);
    return structure;
  }
  ...
  getHowWorksStructure(kenticoItem: any) {
    ...
    const howWorkStructure = {
      coverage: kenticoItem.how_works.value.map(items => {
        return {
  ...
  getContent(codes: string[]): Observable<ContentInterface> {
    ...
    return contentFromKentico$.pipe(switchMap((contentFromKentico) => {
      ...
      obj.container_5 = Object.assign({}, contentFromKentico.contact_us);
