...
export class PreventivatoreYoloSciContentProvider implements PreventivatoreContentProvider {
  ...
  createContentStructureFromKenticoItem(kenticoItem: any) {
    const structure = {
      ...
      how_works_sci: this.getHowWorksStructure(kenticoItem),
  ...
  getHowWorksStructure(kenticoItem: any) {
    ...
    const howWorks = {
      title_section: kenticoItem.how_works_sci.value[0].title.value,
      text_section: kenticoItem.how_works_sci.value[0].text.value,
  ...
  getPolicyCoverages(kenticoItem: any) {
    ...
    kenticoItem.how_works_sci.value[0].benefits.value.map((benefit) => {
      const policyCoverage = {
        name: benefit.product_tab_title.value,
        icon: benefit.icon_trophy.value[0] ? benefit.icon_trophy.value[0].url : null,
        product_code: benefit.system.codename.endsWith('sci_gold') ? 'erv-mountain-gold' : 'erv-mountain-silver',
        icon_trophy: benefit.icon_trophy.value.length > 0 ? benefit.icon_trophy.value[0].url : null,
        box_img: kenticoItem.how_works_sci.value[1].image.value[0].url,
        items_list: benefit.guarantee_list.value.map((guarantee) => {
          return {
            text: guarantee.guarantee_title.value,
            img: guarantee.icon_guarantee.value[0].url,
            list_elements: guarantee.guarantee_list.value.map((subitem) => {
              return {
                list_item: subitem.text.value,
                included: subitem.included.value.length > 0 ? 'included' : 'excluded',
              };
            }),
          };
        }),
  ...
  getContent(codes: string[]): Observable<ContentInterface> {
    ...
    return contentFromKentico$.pipe(switchMap((contentFromKentico) => {
      ...
      obj.container_2 = Object.assign({});
      obj.container_3 = Object.assign({}, contentFromKentico.how_works_sci);
  ...
  private getContentFromKentico(): Observable<any> {
    return this.kenticoTranslateService.getItem('preventivatore_sci').pipe(map(contentItem => this.createContentStructureFromKenticoItem(contentItem)));
  }
