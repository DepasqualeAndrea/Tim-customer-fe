...
export class PreventivatoreYoloSmartphoneContentProvider implements PreventivatoreContentProvider {
  ...
  createContentStructureFromKenticoItem(codes: string[], kenticoItem: any) {
    ...
    kenticoItem.quotator.value.find(item => item.system.codename === 'quotator_errors_smartphone').linked_items.value.forEach(
    ...
    const structure = {
      ...
      quotator: {
        form___value: kenticoItem.quotator.value.find(item => item.system.codename === 'form___value').text.value,
        form_select: kenticoItem.quotator.value.find(item => item.system.codename === 'form_select').text.value,
        form_brand: kenticoItem.quotator.value.find(item => item.system.codename === 'form_brand').text.value,
        form_model: kenticoItem.quotator.value.find(item => item.system.codename === 'form_model').text.value,
        errors: quotatorErrors,
        price_title: kenticoItem.quotator.value.find(item => item.system.codename === 'price').text.value,
        slash_price: kenticoItem.quotator.value.find(item => item.system.codename === 'slash_month').text.value,
        information_package_text: kenticoItem.quotator.value.find(item => item.system.codename === 'product_collaboration_covercare').info_package_link.value,
        product_collaboration: kenticoItem.quotator.value.find(item => item.system.codename === 'product_collaboration_covercare').product_collaboration.value,
        provider_logo: kenticoItem.quotator.value.find(item => item.system.codename === 'product_collaboration_covercare').provider_logo.value[0].url,
        continue_button_label: kenticoItem.quotator.value.find(item => item.system.codename === 'buy_button').text.value,
      },
  ...
  getContent(codes: string[]): Observable<ContentInterface> {
    ...
    return forkJoin([contentFromKentico$, filteredProducts$]).pipe(
      switchMap(([contentFromKentico, filteredProducts]) => {
        ...
        contentFromKentico.what_to_know.terms_of_service_content = PreventivatoreDynamicSharedFunctions.replaceInformationPackageLink(
          contentFromKentico.what_to_know.terms_of_service_content,
  ...
  private getContentFromKentico(codes: string[]): Observable<any> {
    return this.kenticoTranslateService.getItem('preventivatore_yolo_smartphone').pipe
