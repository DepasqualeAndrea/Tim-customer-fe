...
export class PreventivatoreTimCustomersMyHomeFtthContentProviderService implements PreventivatoreContentProvider {
  ...
  private getContentFromKentico(): Observable<any> {
    return this.kenticoTranslateService.getItem('preventivatore_tim_my_home_ftth').pipe
  ...
  createContentStructureFromKenticoItem(kenticoItem: any) {
    const structure = {
      hero: {
        ...
        warranty_description:         kenticoItem.warranty_description_ftth.value,
      },
      ...
      more_information: {
        title: kenticoItem.title_1.value,
        links: this.getLink(kenticoItem.link.value)
      },
      accordions_section: {
        accordions: this.getWhatToKnowCards(kenticoItem.accordion_list.value),
        accordionsFaq: this.getWhatToKnowCards(kenticoItem.accordions_faq.value)
      },
      prefooter_section: {
  ...
  private setContentToProduct(contentFromInsuranceService: any, hero: any) {
    const product = contentFromInsuranceService.find(product => product.product_code === 'tim-my-home');
    const obj = {
      product: product,
      title: hero.title,
      ...
      warranty_description: hero.warranty_description,
    };
    return obj;
  }
  ...
  getContent(codes: string[]): Observable<ContentInterface> {
    ...
    return contentFromKentico$.pipe(switchMap((contentFromKentico) => {
      ...
      obj.container_3 = Object.assign({}, contentFromKentico.more_information, contentFromKentico.accordions_section);
      obj.container_4 = Object.assign({}, contentFromKentico.prefooter_section);
